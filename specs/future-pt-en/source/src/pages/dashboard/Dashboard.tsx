...
export const Dashboard = () => {
  ...
  const barData = jobs.slice(0, 20).map(j => ({
    ...
    fullName: j.name,
    type: j.type === 'job' ? t('vaga') : t('analise'),
  ...
  const labelMap: Record<string, string> = { Vagas: t('vagas'), Analises: t('analises'), Match: t('match') };
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        <p style={{ color: 'var(--text-dim)', fontSize: 13 }}>{t('carregando')}</p>
      </div>
    </div>
  );
  ...
  if (error) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: 40, textAlign: 'center', maxWidth: 400 }}>
        <p style={{ color: '#ef4444', fontWeight: 600, marginBottom: 8 }}>{t('erroCarregar')}</p>
        <p style={{ color: 'var(--text-dim)', fontSize: 13, marginBottom: 20 }}>{error}</p>
        <button onClick={() => fetchData(profile.userId)} style={{ background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 24px', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>{t('tentarNovamente')}</button>
      </div>
    </div>
  );
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '20px' : '16px', marginBottom: '6px' }}>
          ...
          <div style={{ marginLeft: isMobile ? 'auto' : 'auto', display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12, flexShrink: 0, marginRight: isMobile ? '100px' : '0' }}>
            ...
            >
              ...
              {isCustomizing ? t('concluirOnboarding') : 'Customizar'}
            </button>
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 4 : 8, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: isMobile ? 10 : 12, padding: isMobile ? '6px 8px' : '8px 14px', whiteSpace: 'nowrap' }}>
              ...
              <span style={{ color: '#22c55e', fontSize: isMobile ? 11 : 12, fontWeight: 600 }}>{t('tempoReal')}</span>
            </div>
          </div>
        </div>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px', margin: 0 }}>{t('visaoGeral')}</p>
      </div>
      ...
      <div className="anim-2" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: isMobile ? 10 : 16, marginBottom: 24 }}>
        {[
          { 
            label: t('vagas'), value: totalVagas, suffix: '', icon: Briefcase, 
            orb: '#6366f1', 
            planet: { 
              name: 'Saturn', size: 90, 
          ...
          { 
            label: t('candidatosAvaliados'), value: totalAvaliados, suffix: '', icon: Users, 
            orb: '#0ea5e9', 
            planet: { 
          ...
          { 
            label: t('melhoresCandidatos'), value: totalMelhores, suffix: '', icon: Award, 
            orb: '#2563eb', 
            planet: { 
              name: 'Earth', size: 85, 
      ...
      {jobs.length === 0 ? (
        <div className="d-card anim-3" style={{ padding: 64, textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 16, fontWeight: 600, marginBottom: 6 }}>{t('nenhumResultado')}</p>
          <p style={{ color: 'var(--text-dim)', fontSize: 13 }}>{t('semDadosDashboard')}</p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24 }}>
            <button onClick={() => navigate('/analise/nova')} style={{ background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: 12, padding: '10px 28px', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>{t('novaAnalise')}</button>
            <button onClick={() => navigate('/vagas/nova')} style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid var(--primary)', color: 'var(--primary)', borderRadius: 12, padding: '10px 28px', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>{t('criarVaga')}</button>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="anim-3" style={{ position: 'relative', display: 'flex', flexDirection: isMobile ? 'column-reverse' : (layout.calendarPos === 'right' ? 'row' : 'row-reverse'), gap: 16, marginBottom: 16 }}>
            {isCustomizing && (
              ...
              >
                <RefreshCw style={{ width: 12, height: 12 }} /> {t('inverterPosicao')}
              </div>
            )}
            ...
            <div className="d-card" style={{ flex: 1, padding: '24px 20px 16px', minWidth: 0 }}>
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 12 : 0, marginBottom: isMobile ? 16 : 24 }}>
                <div>
                  <p style={{ color: 'var(--text-dim)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>{t('tendencias')}</p>
                  <p style={{ color: 'var(--text-main)', fontSize: isMobile ? 17 : 20, fontWeight: 700, whiteSpace: 'nowrap' }}>{t('candidatos')} & {t('match')}</p>
                ...
                <div style={{ display: 'flex', gap: isMobile ? 12 : 16, flexWrap: 'wrap' }}>
                  {[
                    { color: '#6366f1', label: t('vagas') }, 
                    { color: '#a78bfa', label: t('analises') }, 
                    { color: '#22c55e', label: t('match') }
                  ].map(l => (
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                  {payload.map((p: Record<string, unknown>) => (
                                    <p key={p.dataKey as string} style={{ color: p.color as string, fontSize: 12, fontWeight: 600 }}>
                                      {String(labelMap[p.name as string] || p.name)}: {String(p.value)}
                                    </p>
                                  ))}
                                </div>
                              </div>
              ...
              {isMobile ? (
                ...
                <>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div>
                      <p style={{ color: 'var(--text-dim)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 2 }}>{t('filtro')}</p>
                  ...
                  <div style={{ marginTop: 'auto', paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      <span style={{ color: 'var(--text-dim)', fontSize: 11 }}>{t('diaComVaga')}</span>
                    </div>
                    {activeStart ? (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ color: 'var(--text-dim)', fontSize: 11 }}>
                          {activeEnd && activeEnd !== activeStart
                            ...
                            : <><b style={{ color: 'var(--primary)' }}>{fmtDate(activeStart)}</b>{!rangeEnd ? <span style={{ color: '#f59e0b' }}> • {t('cliqueFinalizar')}</span> : ''}</>
                          }
                        </span>
                        <button onClick={clearRange} style={{ fontSize: 10, color: '#ef4444', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 6, padding: '2px 8px', cursor: 'pointer', fontWeight: 600 }}>{t('limpar')}</button>
                      </div>
                    ) : (
                      <span style={{ color: 'var(--text-dim)', fontSize: 11 }}>{t('cliqueDia')}</span>
                    )}
                  </div>
                </>
              )}
          ...
          <div className="anim-4" style={{ display: 'flex', flexDirection: isMobile ? 'column' : (layout.rankingPos === 'right' ? 'row' : 'row-reverse'), gap: 16, marginBottom: 16, position: 'relative' }}>
            ...
            <div className="d-card" style={{ flex: 1, padding: '24px 20px 16px', minWidth: 0 }}>
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 10 : 0, marginBottom: isMobile ? 16 : 24 }}>
                <div>
                  <p style={{ color: 'var(--text-dim)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>{t('comparativo')}</p>
                  <p style={{ color: 'var(--text-main)', fontSize: isMobile ? 17 : 20, fontWeight: 700, whiteSpace: 'nowrap' }}>{t('candidatosPorVaga')}</p>
                </div>
                <div style={{ display: 'flex', gap: isMobile ? 12 : 16 }}>
                  {[
                    { color: '#6366f1', label: t('avaliados') },
                    { color: '#22c55e', label: t('match') }
                  ].map(l => (
                    <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: l.color }} />
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div style={{ ...TT, padding: '10px 14px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                  <p style={{ color: '#6366f1', fontSize: 12, fontWeight: 600 }}>{t('total')}: {data.Avaliados}</p>
                                  <p style={{ color: '#22c55e', fontSize: 12, fontWeight: 600 }}>{t('match')}: {data.Match}</p>
                                </div>
                              </div>
                            );
                          }
            ...
            <div className="d-card" style={{ width: isMobile ? '100%' : 360, padding: 24, display: 'flex', flexDirection: 'column', flexShrink: isMobile ? 0 : 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <div>
                  <p style={{ color: 'var(--text-dim)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>{t('rank')}</p>
                  <p style={{ color: 'var(--text-main)', fontSize: 17, fontWeight: 700 }}>{t('maioresMatch')}</p>
                </div>
                <button onClick={() => navigate('/vagas')} style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--primary)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                  {t('verTodas')} <ChevronRight style={{ width: 14, height: 14 }} />
                </button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {topJobs.map((j, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        ...
                        <p style={{ color: 'var(--text-dim)', fontSize: 10 }}>{t('total')}</p>
                      </div>
                    </div>
                  );
                })}
                {topJobs.length === 0 && (
                  <p style={{ color: 'var(--text-dim)', fontSize: 13, textAlign: 'center', padding: '24px 0' }}>{t('semDadosDashboard')}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="anim-5 d-card" style={{ padding: 24, marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <p style={{ color: 'var(--text-dim)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>{t('ultimasAtividades')}</p>
                <p style={{ color: 'var(--text-main)', fontSize: 17, fontWeight: 700 }}>{t('ultimasVagasCriadas')}</p>
              </div>
                    ...
                    <div style={{ position: 'relative', zIndex: 3 }}>
                      ...
                      <div style={{ marginBottom: 10 }}>
                        <span style={{ 
                          ...
                          {j.type === 'job' ? t('vaga') : t('analise')}
                        </span>
                      </div>
                      <div style={{ display: 'flex', gap: 16, marginBottom: 8 }}>
                        <div>
                          <p style={{ color: 'var(--text-dim)', fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 1 }}>{t('total')}</p>
                          <p style={{ color: 'var(--text-main)', fontSize: 14, fontWeight: 800 }}>{j.totalCandidates}</p>
                        </div>
                        <div>
                          <p style={{ color: 'var(--text-dim)', fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 1 }}>{t('match')}</p>
                          <p style={{ color: '#22c55e', fontSize: 14, fontWeight: 800 }}>{j.topCandidates}</p>
                        </div>
                        <div style={{ flex: 1, minWidth: 45 }}>
                          <p style={{ color: 'var(--text-dim)', fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 1 }}>{t('porcentagem')}</p>
