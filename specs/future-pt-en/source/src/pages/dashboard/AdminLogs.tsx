            ...
            const orgs = data
                ...
                .reduce((acc: {id: string, name: string}[], l) => {
                    ...
                    if (!acc.find(o => o.id === orgId)) {
                        acc.push({
                            id: orgId,
                            name: l.profiles!.organization_name || `${t('orgPrefix')} ${orgId.slice(0, 5)}`
                        });
                    }
                    return acc;
                }, []);
            ...
            <div style={{ marginBottom: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: '14px', margin: 0 }}>
                        {filteredLogs.length} {filteredLogs.length !== 1 ? t('logsPlural') : t('logsSingular')} {filteredLogs.length !== 1 ? t('encontrados') : t('encontrado')}
                    </p>
                </div>
            </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', flex: 1 }}>
                    ...
                    {isMobile ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
                            <span style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600 }}>{t('periodo')}:</span>
                    ...
                    {!isMobile && <span style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600 }}>{t('filtrarPor')}:</span>}
                            ...
                            <div className="cs-trigger" onClick={() => setIsOrgOpen(!isOrgOpen)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
                                    ...
                                    <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                        {organizations.find(o => o.id === selectedOrgId)?.name || t('todasOrganizacoes')}
                                    </span>
                                </div>
                    ...
                    <div className="cs-container" ref={statusRef} style={{ width: isMobile ? '100%' : '160px' }}>
                        <div className="cs-trigger" onClick={() => setIsStatusOpen(!isStatusOpen)}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                />
                                <span>{statusFilter === 'success' ? t('sucesso') : statusFilter === 'error' ? t('erro') : t('todosStatus')}</span>
                            </div>
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
                {filteredLogs.length === 0 ? (
                    <div style={{ padding: isMobile ? '32px 16px' : '64px', textAlign: 'center', color: 'var(--text-dim)' }}>
                        ...
                        <p style={{ fontSize: '16px' }}>{t('semLogsFiltro')}</p>
                    </div>
                ) : isMobile ? (
                    <>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', tableLayout: 'fixed' }}>
                                ...
                                <thead>
                                    <tr style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
                                        <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'left' }}>{t('dataHora')}</th>
                                        <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'left' }}>{t('usuario')}</th>
                                        <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'left' }}>{t('acao')}</th>
                                        <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'left' }}>{t('detalhes')}</th>
                                        <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center' }}>{t('statusErroHeader')}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {paginatedLogs.map(log => (
                                        <tr key={log.id} style={{ borderBottom: '1px solid var(--border)', fontSize: '13px', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                                            ...
                                            <td style={{ padding: '16px', verticalAlign: 'middle', textAlign: 'left' }}>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                    <span style={{ color: 'var(--text-main)', fontWeight: 600, fontSize: 13 }}>{log.profiles?.name || t('sistema')}</span>
                                                ...
                                                <div style={{ display: 'inline-flex', justifyContent: 'center', width: '100%' }}>
                                                    {log.error ? (
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#ef4444', background: 'rgba(239,68,68,0.1)', padding: '6px 10px', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.2)', fontSize: '12px' }}>
                                                            <AlertCircle size={14} />
                                                            <span style={{ fontWeight: 500 }}>{t('erro')}</span>
                                                        </div>
                                                    ) : (
            ...
            {totalPages > 1 && (
                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '12px 14px' : '14px 20px', borderTop: '1px solid var(--border)', background: 'var(--bg-card)', gap: isMobile ? '8px' : 0 }}>
                    <span style={{ fontSize: 13, color: 'var(--text-dim)' }}>
                        {t('pagina')} {currentPage} {t('de')} {totalPages} · {filteredLogs.length} {t('logsSingular')}
                    </span>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        ...
                        >
                            <ChevronLeft style={{ width: 15, height: 15 }} /> {!isMobile && t('anterior')}
                        </button>
                        ...
                        >
                            {!isMobile && t('proximo')} <ChevronRight style={{ width: 15, height: 15 }} />
                        </button>
                    </div>
                </div>
            )}
                ...
                <div onClick={() => setSelectedLog(null)}
                    ...
                    <div onClick={e => e.stopPropagation()}
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <div style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{t('usuario')}</div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: 'var(--bg-main)', borderRadius: 10, border: '1px solid var(--border)' }}>
                                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                                    {(selectedLog.profiles?.name || t('sistema'))[0].toUpperCase()}
                                </div>
                                <div style={{ minWidth: 0 }}>
                                    <div style={{ color: 'var(--text-main)', fontWeight: 600, fontSize: 14 }}>{selectedLog.profiles?.name || t('sistema')}</div>
                        ...
                        {/* Ação */}
                        <div style={{ marginBottom: 16 }}>
                            <div style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{t('acao')}</div>
                        ...
                        {/* Status + Erro */}
                        <div style={{ marginBottom: 16 }}>
                            <div style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{t('status')}</div>
                            <div style={{ padding: '12px 14px', background: 'var(--bg-main)', borderRadius: 10, border: '1px solid var(--border)' }}>
                                {selectedLog.error ? (
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#ef4444', marginBottom: 8 }}>
                                            <AlertCircle size={16} />
                                            <span style={{ fontWeight: 600, fontSize: 13 }}>{t('erro')}</span>
                                        </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#10b981' }}>
                                        ...
                                        <span style={{ fontWeight: 600, fontSize: 13 }}>{t('sucesso')}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {/* Detalhes */}
                        <div>
                            <div style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{t('detalhes')}</div>
                            <div style={{ padding: '12px 14px', background: 'var(--bg-main)', borderRadius: 10, border: '1px solid var(--border)' }}>
