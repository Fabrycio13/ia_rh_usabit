...
export const SetPassword = () => {
    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(({ data: { session }, error }) => {
            if (error || !session) {
                toast.error(t('linkInvalidoConvite'));
    ...
    const handleSetPassword = async (e: React.FormEvent) => {
        ...
        if (password.length < 6) {
            setMessage({ type: 'error', text: t('senhaMin6Caracteres') });
            return;
        }
        if (password !== confirm) {
            setMessage({ type: 'error', text: t('senhasNaoConferem') });
            return;
        }
        ...
        if (error) {
            setMessage({ type: 'error', text: t('erroDefinirSenha').replace('{msg}', error.message) });
            return;
        }
        ...
        toast.success(t('senhaDefinidaSucesso'));
                ...
                <div className="w-full md:w-[380px] lg:w-[400px] xl:w-[420px] flex-shrink-0 h-full flex flex-col justify-between p-8 md:p-10 bg-[#121316] z-10 overflow-y-auto">
                    ...
                    <div className="my-auto w-full max-w-[320px] mx-auto flex flex-col justify-center">
                        ...
                        <form onSubmit={handleSetPassword} className="flex flex-col gap-5.5">
                            ...
                            >
                                {saving ? t('definindo') : t('definirSenha')}
                            </button>
                        </form>
                    </div>
