...
export const Register = () => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        if (password !== confirmPassword) {
            setMessage({ type: 'error', text: t('asSenhasNaoCoincidem') });
            return;
        }

        if (honeypot) {
            setMessage({ type: 'error', text: t('erroValidacao') });
            return;
        }

        if (isDisposableEmail(email)) {
            setMessage({ type: 'error', text: t('emailTemporario') });
            return;
        }
        ...
        if (error) {
            setMessage({ type: 'error', text: `${t('erroCadastro')} ${error.message}` });
        } else {
                ...
                <div className="w-full md:w-[380px] lg:w-[400px] xl:w-[420px] flex-shrink-0 h-full flex flex-col justify-between p-8 md:p-10 bg-[#121316] z-10 overflow-y-auto">
                    ...
                    <div className="my-auto w-full max-w-[320px] mx-auto flex flex-col justify-center">
                        ...
                        <form onSubmit={handleRegister} className="flex flex-col gap-4">
                            ...
                            >
                                {loading ? t('criarConta') + '...' : cooldown > 0 ? `${t('aguarde')} ${cooldown}s` : t('criarConta')}
                            </button>
