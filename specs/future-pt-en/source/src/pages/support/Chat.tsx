...
export function Chat() {
    ...
    async function loadConversations() {
        ...
        try {
            ...
            const mapped: Conversation[] = (data || []).map((row: ConversationQueryRow) => {
                ...
                return {
                    id: row.candidate_id,
                    name: c?.name || t('candidatoDesconhecido'),
                    ...
                    preview: lastMsg?.text || t('novaConversaAtivada'),
                    unread: 0,
                    candidate: c?.name || '',
                    vaga: 'WhatsApp',
                    score: 0,
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            alert(t('falhaEnvioMensagem').replace('{msg}', (err as Error).message));
        } finally {
            setSending(false);
        }
    };
    ...
    const filterLabels: Record<string, string> = { 'todos': t('todosFiltro'), 'nao-lidos': t('naoLidos'), 'agendados': t('agendados') };
                ...
                <div style={{
                    ...
                    <div style={{ padding: '18px 16px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <div style={{
                            ...
                            <input
                                type="text"
                                placeholder={t('buscarContatoVaga')}
                                value={search}
                    ...
                    {activeConv ? (
                        <>
                            ...
                            <div style={{
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ fontSize: '11px', color: activeConv.online ? '#22c55e' : 'var(--text-dim)', marginTop: '1px' }}>
                                        {activeConv.online ? t('onlineAgora') : t('vistoUltimoHoje')}
                                    </div>
                                </div>
                            ...
                            <div style={{
                                ...
                                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{t('candidatoLabel')}</span>
                             ...
                             <div style={{
                                 ...
                                 {!hasCredentials ? (
                                     <div style={{ flex: 1, color: '#f59e0b', fontSize: 12, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}>
                                         <Zap size={14} /> {t('configureApiMensagem')}
                                     </div>
                                 ) : (
                                     <>
                                         <input
                                             className="chat-input"
                                             type="text"
                                             placeholder={t('carregarMensagemPlaceholder')}
                                             value={inputText}
                ...
                <div className="modal-overlay" onClick={() => setShowNewChatModal(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        <div style={{ padding: '20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: 'var(--text-main)' }}>{t('novaConversaModal')}</h3>
                        ...
                        <div style={{ padding: '16px', borderBottom: '1px solid var(--border)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: '10px', padding: '8px 12px' }}>
                                ...
                                <input 
                                    type="text" 
                                    placeholder={t('buscarCandidatoPlaceholder')} 
                                    value={candSearch}
                        ...
                        <div className="custom-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '10px 0' }}>
                            {loadingCands ? (
                                ...
                                <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-dim)', fontSize: '13px' }}>{t('nenhumCandidatoEncontrado')}</div>
