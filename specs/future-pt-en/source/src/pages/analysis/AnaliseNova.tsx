        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ padding: '32px 28px 24px', borderBottom: '1px solid var(--border)' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            ...
                            <div>
                                ...
                                <span style={{
                                    ...
                                    {candidate.source === 'excel' ? t('uploadViaExcel') : t('uploadViaPdf')}
                                </span>
                            </div>
                        </div>
                        <button
                    ...
                    <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#94a3b8' }}>
                            <MapPin size={13} />{candidate.location ?? t('naoInformado')}
                        </span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#94a3b8' }}>
                            <Calendar size={13} />{(candidate.age && !['Não informado', 'não informado', '—'].includes(candidate.age)) ? `${String(candidate.age).replace(/\s*anos?/i, '')} ${t('anos')}` : t('naoInformado')}
                        </span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#94a3b8' }}>
                            <UserRound size={13} />{candidate.gender ?? t('naoInformado')}
                        </span>
                    </div>
                </div>
                ...
                <div style={{ padding: '16px 28px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 13, color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('score')}</span>
                    <span style={{
                ...
                <div style={{ flex: 1, padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                    {/* Contato */}
                    <section>
                        <p style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>{t('informacoesContato')}</p>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
                            {[
                                { icon: <Mail size={14} />, label: t('email'), value: candidate.email },
                                { icon: <Phone size={14} />, label: t('phone'), value: candidate.phone },
                                { icon: <MapPin size={14} />, label: t('localizacao'), value: candidate.location },
                                { icon: <Calendar size={14} />, label: t('idade'), value: (candidate.age && !['Não informado', '—'].includes(candidate.age)) ? `${String(candidate.age).replace(/\s*anos?/i, '')} ${t('anos')}` : null },
                                { icon: <UserRound size={14} />, label: t('genero'), value: candidate.gender },
                            ].map((item, idx) => (
                                <div key={idx} style={{
                                    ...
                                    <span style={{ fontSize: 13, color: item.value ? 'var(--text-main)' : 'var(--text-muted)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.value ?? t('naoInformado')}</span>
                                </div>
                            ))}
                        </div>
                    </section>
                    ...
                    {candidate.skills && (
                        <section>
                            <p style={{ fontSize: 12, color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>{t('habilidades')}</p>
                    ...
                    {candidate.experience && (
                        <section>
                            <p style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{t('experiencia')}</p>
                    ...
                    {candidate.education && (
                        <section>
                            <p style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{t('educacao')}</p>
                    ...
                    {/* Pontos de Atenção */}
                    <section>
                        <p style={{ fontSize: 12, color: '#ef4444', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{t('pontosAtencao')}</p>
                        <div style={{ padding: '0 0' }}>
                            {candidate.attention_points && !candidate.attention_points.includes('Não existem pontos de atenção') ? (
                                ...
                                <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: 0 }}>{t('nenhumResultado')}</p>
                            )}
                        </div>
                    </section>
                </div>
...
function ResultsView({ result, jobName, onBack, onExit, userId }: { result: AnalysisResult; jobName: string; onBack: () => void; onExit: () => void; userId: string }) {
    ...
    const tabs = [
        { key: 'best' as const, label: t('melhores'), count: best.length, color: '#10b981' },
        { key: 'mid' as const, label: t('intermediarios'), count: mid.length, color: '#f59e0b' },
        { key: 'worst' as const, label: t('piores'), count: worst.length, color: '#ef4444' },
    ];
    ...
    return (
        <div style={{ background: 'var(--bg-main)', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
            ...
            <div style={{ padding: '24px 40px 0' }}>
                <div style={{
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        >
                            <Star size={14} fill={activeTab === 'best' ? '#818cf8' : 'none'} />{t('analiseIA')} ({best.length})
                        </button>
                        <button
                            onClick={onExit}
                            style={{
            ...
            <div style={{ padding: '40px 40px 80px' }}>
                {/* Header */}
                <h1 style={{ color: 'var(--text-main)', fontSize: 28, fontWeight: 700, marginBottom: 8 }}>{t('candidatos')}</h1>
                    ...
                    {tabs.map(tab => (
                        <button
                            ...
                            style={{
                                padding: '10px 28px',
                                borderRadius: 14,
                                border: `1px solid ${activeTab === tab.key ? tab.color : `${tab.color}33`}`,
                                cursor: 'pointer',
                                fontSize: 14,
                                fontWeight: 700,
                            ...
                            onMouseEnter={e => {
                                if (activeTab !== tab.key) {
                                    e.currentTarget.style.background = `${tab.color}11`;
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 40 }}>
                    ...
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '20px 16px' }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-main)', marginBottom: 4 }}>{t('genero')}</p>
                    ...
                    {/* Age Bar */}
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '20px 16px' }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-main)', marginBottom: 4 }}>{t('idade')}</p>
                        <div style={{ height: 200 }}>
                            <ResponsiveContainer width="100%" height="100%">
                    ...
                    {/* Location */}
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '20px 16px' }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-main)', marginBottom: 4 }}>{t('localizacao')}</p>
                        <div style={{ height: 200 }}>
                            <ResponsiveContainer width="100%" height="100%">
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                        ...
                        <thead>
                            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-card)' }}>
                                {[t('rank'), t('nome'), t('idade'), t('localizacao'), t('genero'), t('score'), t('acoes')].map(h => (
                        ...
                        <tbody>
                            {displayed.length === 0 ? (
                                <tr><td colSpan={7} style={{ padding: '40px 20px', textAlign: 'center', color: '#64748b', fontSize: 14 }}>
                                    {t('nenhumCandidatoNestaCategoria')}
                                </td></tr>
                            ) : displayed.map((c) => (
                                <tr key={c.id}
                                ...
                                >
                                    ...
                                    <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--text-dim)' }}>
                                        {(c.age && !/(não|nao)\s*informado|—/i.test(c.age)) ? `${String(c.age).replace(/\s*anos?/i, '').trim()} ${t('anos')}` : '—'}
                                    </td>
                                    {/* Localização */}
                                    ...
                                    <td style={{ padding: '14px 16px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                            ...
                                            <button
                                                title={c.resumeUrl ? t('abrirCurriculo') : t('pdfNaoDisponivel')}
                                                onClick={e => {
                                                    e.stopPropagation();
                                                    if (c.resumeUrl) {
                                                        handleViewResume(c.resumeUrl);
                                                    } else {
                                                        toast.error(t('pdfNaoDisponivel'));
                                                    }
                                                }}
...
export const AnaliseNova = () => {
    ...
    // If "new=true" in URL, clear existing analysis and remove param
    useEffect(() => {
    ...
    const handleAnalyze = async () => {
        if (!jobName.trim()) { setFormError(t('erroSalvar')); return; }
        if (files.length === 0) { setFormError(t('erroUpload')); return; }
        setFormError(null);
    ...
    if (!analyzing && result) {
        return <ResultsView
            result={result}
            jobName={jobName || ctxJobName || t('analise')}
            onBack={() => {
                clearAnalysis();
                setJobName('');
                setJobDesc('');
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <h1 style={{ color: 'var(--text-main)', fontSize: 32, fontWeight: 800, margin: 0 }}>
                            {analyzing ? t('analisando') : (jobName || t('novaAnalise'))}
                        </h1>
                    </div>
                    {analyzing && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <div style={{ color: '#64748b', fontSize: 13, fontWeight: 500 }}>
                                {progress.current} / {progress.total} {t('processando')}
                            </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0, 1.6fr) minmax(0, 1fr)', gap: isMobile ? 16 : 32, alignItems: 'stretch' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ padding: '24px 24px 16px' }}>
                            <label style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8', display: 'block', marginBottom: 8 }}>{t('nomeDaVaga')}</label>
                            <input
                                value={jobName}
                                onChange={e => setJobName(e.target.value)}
                                disabled={analyzing}
                                placeholder={t('exTituloVaga')}
                                style={{
                        ...
                        <div style={{ padding: '0 24px 16px' }}>
                            <label style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8', display: 'block', marginBottom: 8 }}>{t('descricaoVaga')}</label>
                            <textarea
                                value={jobDesc}
                                onChange={e => setJobDesc(e.target.value)}
                                disabled={analyzing}
                                placeholder={t('descrevaRequisitos')}
                                rows={12}
                                style={{
                                    width: '100%', background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 10,
                        ...
                        <div style={{ padding: '16px 24px 24px' }}>
                            <label style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8', display: 'block', marginBottom: 12 }}>{t('uploadCurriculo')}</label>
                                ...
                                <div style={{ background: '#0d1020', border: '1px solid #2d3a6e', borderRadius: 10, padding: 16, marginBottom: 14 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                                        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: '#818cf8' }}>
                                            <FileSpreadsheet size={14} /> {t('excelPadronizado')}
                                        </span>
                                        <button
                                        ...
                                        >
                                            <Download size={11} /> {t('baixar')}
                                        </button>
                                    </div>
                                    <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 10, lineHeight: 1.5 }}>
                                        {t('instrucoesExcel')}
                                    </p>
                                    <p style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{t('colunasObrigatorias')}</p>
                                    {[
                                        { key: t('fullName'), desc: t('candidato') },
                                        { key: t('email'), desc: t('email') },
                                        { key: t('phone'), desc: t('phone') },
                                        { key: t('experiencia'), desc: t('experiencia') },
                                        { key: t('educacao'), desc: t('educacao') },
                                    ].map(c => (
                                        <p key={c.key} style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>
                                            <strong style={{ color: 'var(--primary)' }}>{c.key}</strong> – {c.desc}
                                        </p>
                                    ))}
                                </div>
                            )}
                            ...
                            {!profile.isPremium && monthlyJobCount >= 20 && (
                                <div style={{ marginTop: 12, padding: '10px', background: '#f59e0b15', borderRadius: 8, border: '1px solid #f59e0b33', color: '#f59e0b', fontSize: 12, textAlign: 'left' }}>
                                    ⚠️ <strong>{t('limiteAtingido')}</strong> {t('upgradeParaContinuar')}
                                </div>
                            )}
                            ...
                            >
                                {analyzing ? (
                                    <><div style={{ width: 16, height: 16, border: '2px solid #fff', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                                        {t('analisando')} {progress.total > 0 ? `${progress.current} ${t('deProgresso')} ${progress.total}` : '...'}</>
                                ) : (!profile.isPremium && monthlyJobCount >= 20) ? (
                                    <><Trophy size={16} /> {t('limiteMensalAtingido')}</>
                                ) : (
                                    <><Zap size={16} />{t('analisar')}</>
                                )}
                            </button>
                        </div>
                    </div>
                    ...
                    <div style={{
                        ...
                        {(analyzing || (result?.candidates?.length ?? 0) > 0) ? (
                            <div style={{ display: 'flex', flexDirection: 'column', height: '100%', maxHeight: 800 }}>
                                <div style={{ padding: '24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-main)' }}>
                                    <div style={{ textAlign: 'left' }}>
                                        <h3 style={{ color: 'var(--text-main)', fontSize: 16, fontWeight: 700, margin: 0 }}>{t('candidatos')}</h3>
                                        <p style={{ fontSize: 12, margin: '4px 0 0', color: 'var(--text-dim)' }}>
                                            {result?.candidates?.length || 0} {t('curriculo')}
                                        </p>
                                    </div>
                                    {analyzing && (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#818cf8', background: '#1a2350', padding: '6px 12px', borderRadius: 8, border: '1px solid #2d3a6e' }}>
                                            ...
                                            <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('analiseIA')}</span>
                                        </div>
                                    )}
                                </div>
                                <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }} className="custom-scrollbar">
                                    {(result?.candidates || []).slice().reverse().map((c, idx) => (
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1, textAlign: 'left', minWidth: 0 }}>
                                                ...
                                                <p style={{ color: 'var(--text-dim)', fontSize: 12, margin: '4px 0 0' }}>{c.gender ?? t('naoInformado')} • {c.location ?? t('naoInformado')}</p>
                                            </div>
                                    ...
                                    {analyzing && (
                                        <div style={{ padding: '32px 20px', textAlign: 'center' }}>
                                            ...
                                            <p style={{ fontSize: 13, color: '#94a3b8', fontWeight: 500 }}>{t('carregando')}</p>
                                            <p style={{ fontSize: 11, color: '#4a5568', marginTop: 8 }}>{t('extraindoDados')}</p>
                                        </div>
                                    )}
                                </div>
                                {analyzing && (
                                    <div style={{ padding: '20px 24px', borderTop: '1px solid #1f2332', background: 'rgba(99,102,241,0.03)', textAlign: 'left' }}>
                                        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                                            ...
                                            <p style={{ fontSize: 12, color: '#94a3b8', lineHeight: '1.5', margin: 0 }}>
                                                <strong>{t('dica')}</strong> {t('dicaNavegar')}<br />{t('dicaProcessamento')}
                                            </p>
                                        </div>
                                    </div>
                                )}
                            ...
                            <>
                                ...
                                <h3 style={{ color: 'var(--text-main)', fontSize: 18, fontWeight: 700, margin: '0 0 12px' }}>{t('analise')}</h3>
