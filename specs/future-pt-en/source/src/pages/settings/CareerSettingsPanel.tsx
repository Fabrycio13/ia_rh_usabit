...
export const CareerSettingsPanel: React.FC<CareerSettingsPanelProps> = ({ showToast }) => {
    ...
    useEffect(() => {
        const fetchOrgSettings = async () => {
            ...
            showToast('error', t('erroCarregarConfigs'));
        } finally {
            setLoading(false);
        }
    };
    fetchOrgSettings();
}, [profile.organization_id, showToast, t]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (error) throw error;
            showToast('success', t('configsSalvasSucesso'));
        } catch (err: unknown) {
            console.error('[CareerSettings] Erro ao salvar:', err);
            showToast('error', t('erroSalvarConfigs').replace('{msg}', err instanceof Error ? err.message : String(err)));
        } finally {
            setSaving(false);
        }
    };
    ...
    if (!profile.organization_id) {
        return (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '40px', textAlign: 'center' }}>
                <p style={{ color: 'var(--text-dim)', fontSize: '14px' }}>{t('semOrganizacao')}</p>
            </div>
        );
    }
                ...
                <div style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '20px' }}>
                    ...
                    <button
                        onClick={() => {
                            ...
                            showToast('success', t('linkCopiadoArea'));
                        }}
            ...
            <div style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div>
                    <label style={labelStyle}>
                        ...
                        {t('sobreAEmpresa').replace('{orgName}', orgName || t('empresa'))}
                    </label>
                    <textarea
                        style={{ ...inputStyle, minHeight: '100px', resize: 'vertical' }}
                        placeholder={t('sobrePlaceholder')}
                        value={aboutText}
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
                    ...
                    >
                        ...
                        {saving ? t('salvando') : t('salvarConfiguracoes')}
                    </button>
                </div>
            </div>
        </div>
