...
export const OwnerAdminApiPanel = ({
    ...
    const handleSave = async (adminId: string) => {
        ...
        setSaving(null);
        if (error) {
            toast.error(t('erroSalvar') + ': ' + error.message);
        } else {
            toast.success(t('alteracoesSalvas'));
        }
    };
    ...
    if (gestores.length === 0) {
        return (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '40px 28px', textAlign: 'center' }}>
                <p style={{ color: 'var(--text-dim)', fontSize: '14px' }}>{t('nenhumRegistro')}</p>
            </div>
        );
    }
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                                <span style={{
                                    ...
                                    {hasConfig ? t('apiConfigurado') : t('apiSemConfig')}
                                </span>
                        ...
                        {isOpen && (
                            <div style={{ padding: isMobile ? '0 14px 14px' : '0 20px 20px', borderTop: '1px solid var(--border)' }}>
                                <div style={{ paddingTop: '16px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '2fr 1.5fr 1fr', gap: '14px' }}>
                                    <div>
                                        <label style={labelStyle}>{t('serverUrl')}</label>
                                    ...
                                    <div>
                                        <label style={labelStyle}>{t('instancia')}</label>
                                        <div style={fieldWrapStyle}>
                                ...
                                <div style={{ marginTop: '14px', display: 'flex', justifyContent: 'flex-end' }}>
                                    ...
                                    >
                                        ...
                                        {saving === admin.id ? t('saving') : t('salvar')}
                                    </button>
                                </div>
                            </div>
                        )}
...
export const OwnerAdminPlanPanel = ({
    ...
    const handleSave = async (adminId: string) => {
        ...
        if (error) {
            ...
            toast.success(t('admPlanoAtualizado'));
            setEditing(null);
        }
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr style={{ borderBottom: '1px solid var(--border)' }}>
                            <th style={{ padding: '14px 20px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'left' }}>{t('admHeaderAdministrador')}</th>
                            <th style={{ padding: '14px 20px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center' }}>{t('admHeaderPlanoAtual')}</th>
                            <th style={{ padding: '14px 20px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center' }}>{t('status')}</th>
                            <th style={{ padding: '14px 20px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center' }}>{t('acoes')}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {gestores.map(admin => {
                                    ...
                                    <td style={{ padding: '14px 20px', textAlign: 'center' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                            ...
                                            <span style={{ fontSize: '12px', color: admin.status === 'active' ? '#10b981' : '#ef4444', fontWeight: 600 }}>
                                                {admin.status === 'active' ? t('active') : t('inativa')}
                                            </span>
                                        </div>
                                    </td>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '10px', marginTop: '4px' }}>
                {([['trial', '#f59e0b', t('admLegendaTrial')], ['pro', '#6366f1', t('admLegendaPro')], ['enterprise', '#10b981', t('sobConsulta')], ['lifetime', '#dc2626', t('admLegendaLifetime')]] as const).map(([key, color, desc]) => (
