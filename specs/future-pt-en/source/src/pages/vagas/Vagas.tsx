...
export const Vagas = ({ hideHeader = false }: { hideHeader?: boolean }) => {
    ...
    const statusConfigMap: Record<VagaStatus, { bg: string; color: string; label: string }> = {
        aberta: { bg: 'rgba(34, 197, 94, 0.1)', color: '#22c55e', label: t('aberta') },
        fechada: { bg: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', label: t('fechada') },
        pausada: { bg: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', label: t('pausada') },
        cancelada: { bg: 'rgba(100, 116, 139, 0.1)', color: '#64748b', label: t('cancelada') },
        invisivel: { bg: 'rgba(99, 102, 241, 0.1)', color: '#6366f1', label: t('invisivel') }
    };
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                ...
                toast.error(t('erroCarregarInfo'));
            } finally {
                setLoading(false);
            }
        };
        ...
        try {
            ...
            if (status === 'aberta' && vagaAtual?.status === 'cancelada') {
                ...
                if (allCandidates.length > 0) {
                    ...
                    toast.success(`${t('statusAlterado')} "${t('aberta')}"`);
                }
                return;
            }
            ...
            const statusLabels = { aberta: t('aberta'), fechada: t('fechada'), pausada: t('pausada'), cancelada: t('cancelada'), invisivel: t('invisivel') };
            toast.success(`${t('statusAlterado')} "${statusLabels[status]}"`);
    ...
    const confirmPipelineDelete = async () => {
        ...
        try {
            ...
            toast.success(t('pipelineExcluido'));
    ...
    async function sendCloseEmails(vagaId: string, vagaTitle: string, tipo: 'fechada' | 'cancelada' = 'fechada') {
        ...
        try {
            ...
            if (!breakdown) {
                toast.success(t('nenhumCandidatoNotificar'));
            ...
            if (failed > 0) {
                toast.success(`${sent} ${t('emailsEnviados')}, ${failed} ${t('envioFalhas')}`);
            } else {
                toast.success(`${sent} ${t('emailsEnviados')}`);
            }
        } catch (err) {
            console.error('Erro ao enviar e-mails:', err);
            toast.error(t('erroEnviarEmails'));
        } finally {
    ...
    async function sendReopenedEmails() {
        ...
        try {
            ...
            if (failed > 0) {
                ...
                toast.success(`${sent} ${t('candidatoNotificado')}`);
            }
        } catch (err) {
        ...
        <div style={{ padding: hideHeader ? '0' : '0 40px 40px' }}>
            ...
            <div style={{
                        ...
                        <div className="cs-trigger" onClick={() => setIsOrgSelectOpen(!isOrgSelectOpen)}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden' }}>
                                ...
                                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                    {selectedOrgId ? organizations.find(o => o.id === selectedOrgId)?.name : t('todasOrganizacoes')}
                                </span>
                            </div>
                            ...
                            <div className="cs-dropdown" style={{ minWidth: '250px' }}>
                                ...
                                <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                                    ...
                                    {roleOptions.length === 0 && (
                                        <div style={{ padding: '12px', textAlign: 'center', color: 'var(--text-dim)', fontSize: 12 }}>{t('nenhumVaga')}</div>
                                    )}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="cs-trigger" onClick={() => setIsStatusSelectOpen(!isStatusSelectOpen)}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <Filter size={16} style={{ color: '#10b981' }} />
                                    <span>
                                {selectedStatusFilter ? statusConfigMap[selectedStatusFilter as VagaStatus].label : t('todosStatus')}
                            </span>
                        </div>
                ...
                <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: 12, marginLeft: isMobile ? 0 : 'auto', flexDirection: isMobile ? 'column' : 'row', width: isMobile ? '100%' : 'auto', order: isMobile ? 2 : 0 }}>
                    <span style={{ fontSize: 13, color: 'var(--text-dim)', fontWeight: 600 }}>{t('periodo')}:</span>
            ...
            <div style={{
                ...
                {loading ? (
                    ...
                    <div style={{
                        ...
                        <p style={{ fontSize: '16px', marginBottom: '8px' }}>{t('nenhumVaga')}</p>
                        <p style={{ fontSize: '14px' }}>{t('adicioneItem')}</p>
                    </div>
                ) : isMobile ? (
                    ...
                    <>
                        ...
                        <div style={{
                            ...
                            <div>{t('codigoVaga')}</div>
                            <div style={{ textAlign: 'center' }}>{t('status')}</div>
                            <div>{t('tituloVaga')}</div>
                            <div>{t('localizacao')}</div>
                            <div style={{ textAlign: 'center' }}>{t('dataEntrada')}</div>
                            <div style={{ textAlign: 'center' }}>{t('modeloContrato')}</div>
                            <div style={{ textAlign: 'center' }}>{t('candidatos')}</div>
                            <div style={{ textAlign: 'center' }}>Link</div>
                            <div style={{ textAlign: 'center' }}>{t('acoes')}</div>
                        </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <button onClick={() => copyPublicLink(vaga.public_hash)} title={t('copiar')} style={{ padding: '6px', background: 'transparent', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <ExternalLink size={14} />
                                        </button>
                                    </div>

                                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', justifyContent: 'center' }}>
                                        <button onClick={() => window.open(getPublicJobUrl(vaga.public_hash, true), '_blank')} title={t('visualizar')} style={{ padding: '6px', background: 'transparent', border: '1px solid rgba(59, 130, 246, 0.3)', borderRadius: '6px', color: '#3b82f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <Eye size={14} />
                                        </button>
                                        <button onClick={() => navigate(`/vagas/${vaga.id}/candidatos`)} title={t('candidatos')} style={{ padding: '6px', background: 'transparent', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '6px', color: '#10b981', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <Users size={14} />
                                        </button>
                                        {userRole !== 'convidado' && (<>
                                        <button onClick={() => navigate(`/vagas/editar/${vaga.id}`)} title={t('editar')} style={{ padding: '6px', background: 'transparent', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '6px', color: '#f59e0b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <Edit size={14} />
                                        </button>
                                        <button onClick={() => openDeleteModal(vaga.id)} title={t('excluir')} style={{ padding: '6px', background: 'transparent', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '6px', color: '#ef4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <Trash2 size={14} />
                                        </button>
                                        </>)}
                                    </div>
                ...
                {!loading && filteredVagas.length > 0 && (
                    <div style={{
                        ...
                        <div style={{ textAlign: isMobile ? 'center' : 'left' }}>
                            {t('pagina')} <strong>{Math.min(filteredVagas.length, (currentPage - 1) * itemsPerPage + 1)}</strong>-
                            <strong>{Math.min(filteredVagas.length, currentPage * itemsPerPage)}</strong> {t('de')} <strong>{filteredVagas.length}</strong> {t('candidatos').toLowerCase()}
                        </div>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '12px' }}>
                            ...
                            >
                                {deleting ? t('excluir') + '...' : t('confirmar')}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div style={{
                        ...
                        <div style={{ position: 'relative', zIndex: 1 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                ...
                                >
                                    {deletingPipeline ? t('excluir') + '...' : t('confirmar')}
                                </button>
                    ...
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 24, padding: isMobile ? 24 : 40, maxWidth: 480, width: isMobile ? '95%' : '90%', boxShadow: '0 24px 48px rgba(0,0,0,0.5)' }}>
                        {closeEmailVagaType === 'cancelada' ? (
                            <>
                                ...
                                <h2 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-main)', marginBottom: 12, textAlign: 'center' }}>{t('enviarEmailsCandidatos')}?</h2>
                                <p style={{ color: 'var(--text-muted)', fontSize: 14, textAlign: 'center', marginBottom: 16 }}>
                                    {t('desejaEnviarEmails')}
                                </p>
                                {closeEmailBreakdown && (
                                    ...
                                    <div style={{ marginBottom: 8, border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                                        <div onClick={() => setOpenSections(s => ({ ...s, approved: !s.approved }))}
                                            ...
                                            <span style={{ fontSize: 13, fontWeight: 700, color: '#22c55e', flex: 1 }}>
                                                {t('aprovado')}: ({closeEmailBreakdown.approved.length})
                                            </span>
                                        </div>
                                    ...
                                    <div style={{ marginBottom: 8, border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                                        <div onClick={() => setOpenSections(s => ({ ...s, rejected: !s.rejected }))}
                                            ...
                                            <span style={{ fontSize: 13, fontWeight: 700, color: '#ef4444', flex: 1 }}>
                                                {t('rejeitado')} ({closeEmailBreakdown.rejected.length})
                                            </span>
                                        </div>
                                    ...
                                    <div style={{ marginBottom: 8, border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                                        <div onClick={() => setOpenSections(s => ({ ...s, others: !s.others }))}
                                            ...
                                            <span style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b', flex: 1 }}>
                                                {t('noBanco')} ({closeEmailBreakdown.others.length})
                                            </span>
                                        </div>
                                        {openSections.others && (
                                            <div style={{ maxHeight: 150, overflowY: 'auto', borderTop: '1px solid var(--border)', padding: '6px 0' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                            ...
                            >
                                {sendingCloseEmails ? t('enviar') + '...' : `${t('sim')}, ${t('enviar')} ${closeEmailVagaCount ?? '...'} ${t('candidatos').toLowerCase()}`}
                            </button>
                            <button
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 24, padding: isMobile ? 24 : 40, maxWidth: 480, width: isMobile ? '95%' : '90%', boxShadow: '0 24px 48px rgba(0,0,0,0.5)' }}>
                        <div style={{ textAlign: 'center', marginBottom: 24 }}>
                            ...
                            <h2 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 800, color: 'var(--text-main)', marginBottom: 12 }}>{t('enviarEmailsCandidatos')}?</h2>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                            ...
                            >
                                {sendingCloseEmails ? t('enviar') + '...' : `${t('sim')}, ${t('enviarNotificacoes').toLowerCase()}`}
                            </button>
                            <button
                                onClick={() => {
                                    if (reopenEmailVagaId) {
