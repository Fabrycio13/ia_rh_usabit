...
const CATEGORY_ORDER = ['design', 'desenvolvimento', 'marketing', 'vendas'];
...
export const VagasSection = ({
  ...
  const { t } = useLang();
  const allLabel = t('todasAsVagas');
  const categories = [
    allLabel,
    ...Array.from(new Set(vagas.map((v) => v.category || FALLBACK_CATEGORY))).sort(
      (a, b) => {
  ...
  const filteredVagas = vagas.filter(
    (v) =>
      ...
      (v.category || FALLBACK_CATEGORY) === activeCategory
  );
      ...
      >
        {categories.map((cat) => {
          ...
          const count = vagas.filter(
            (v) => cat === allLabel || (v.category || FALLBACK_CATEGORY) === cat
          ).length;
          return (
            <button
              key={cat}
