...
export const VagaForm = () => {
    ...
    const handleNext = () => {
        ...
        if (currentStep === 1) {
            if (!formData.title.trim()) {
                toast.error(t('tituloVaga'));
                return;
            }
            if (!formData.category) {
                toast.error(t('areaDepartamento'));
                return;
            }
        }
        ...
        if (currentStep === 2) {
            if (!formData.workModel) {
                toast.error(t('modeloTrabalho'));
                return;
            }
            if ((formData.workModel === 'hybrid' || formData.workModel === 'onsite') && !formData.location.trim()) {
                toast.error(t('localizacao'));
                return;
            }
            if (!formData.contractType) {
                toast.error(t('modeloContrato'));
                return;
            }
            if (!formData.workRegime) {
                toast.error(t('regimeTrabalho'));
                return;
            }
        }
        ...
        if (currentStep === 3) {
            if (!formData.responsibilities.trim()) {
                toast.error(t('responsabilidadesVaga'));
                return;
            }
            if (!formData.requirements.trim()) {
                toast.error(t('requisitosVaga'));
                return;
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!user) {
                toast.error(t('erroAutenticacao'));
                return;
            }
            ...
            if (isEditMode) {
                ...
                toast.success(t('vagaAtualizada'));
                navigate('/vagas');
            } else {
    ...
    const handleCreatePipeline = async () => {
        ...
        try {
            ...
            toast.success(t('pipelineCriado'));
            setShowPipelineModal(false);
            navigate('/vagas');
        } catch (error) {
            console.error('Erro ao criar pipeline:', error);
            const errMsg = error instanceof Error ? error.message : (error as { message?: string }).message || t('erroCriarPipeline');
            toast.error(`${t('erroCriarPipeline')}: ${errMsg}`);
            setShowPipelineModal(false);
            navigate('/vagas');
        } finally {
    ...
    const steps = [
        { number: 1, title: t('informacoesBasicas') },
        { number: 2, title: t('detalhesCargo') },
        { number: 3, title: t('conteudoVaga') },
        { number: 4, title: t('jornadaCandidato') },
    ];
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                        <h1 style={{ fontSize: isMobile ? '24px' : '36px', fontWeight: 800, color: 'var(--text-main)', margin: 0, letterSpacing: '-0.02em' }}>
                            {isEditMode ? t('editarVaga') : t('novaVaga')}
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: isMobile ? '13px' : '15px', margin: 0, maxWidth: '600px' }}>
                            {isEditMode ? t('editarVaga') : t('dadosPrincipais')}
                        </p>
                    </div>
                </div>
            </div>
                            ...
                            <div style={sectionStyle(isMobile)}>
                                            ...
                                            return (
                                                ...
                                                >
                                                    <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                        ...
                                                        {selected ? selected.label : t('selecioneArea')}
                                                    </span>
                                                    <ChevronDown size={18} />
                                                </button>
                                            );
                                ...
                                {!isEditMode && (
                                    <div style={{ marginBottom: '20px' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '8px' }}>
                                            {([
                                                { value: 'aberta', label: t('publicadaVisivel'), desc: t('publicadaDesc'), color: '#22c55e' },
                                                { value: 'invisivel', label: t('invisivel'), desc: t('invisivelDesc'), color: '#6366f1' },
                                            ] as const).map(opt => (
                                                <button
                                                    key={opt.value}
                                                    type="button"
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        placeholder={t('descrevaResponsabilidades')}
                                        rows={6}
                                ...
                                {formData.isThirdParty && (
                                    <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '20px', animation: 'slideDown 0.3s ease-out' }}>
                                        ...
                                        <ToggleField
                                            label={t('exibirNomeEmpresa')}
                                            description={t('logoEmpresa')}
                    ...
                    {currentStep === 2 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.3s ease-out' }}>
                            ...
                            <div style={sectionStyle(isMobile)}>
                                ...
                                <ToggleField
                                    label={t('faixaSalarial')}
                                    description={t('detalhesDaVaga')}
                                        ...
                                        <div style={{
                                            ...
                                            <div>
                                                <p style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: 'var(--text-main)' }}>
                                                    {alreadyAdded ? t('alteracoesSalvas') : t('faixaSalarial')}
                                                </p>
                            ...
                            <div style={sectionStyle(isMobile)}>
                                ...
                                <RadioGroup
                                    label={t('modeloContrato')}
                                    options={[
                                        { value: 'clt', label: t('clt'), description: t('clt') },
                                        { value: 'pj', label: t('pj'), description: t('pj') },
                                        { value: 'estagio', label: t('estagio'), description: t('estagio') },
                                        { value: 'freelancer', label: t('freelancer'), description: t('freelancer') },
                                    ]}
                            ...
                            <div style={sectionStyle(isMobile)}>
                                ...
                                <RadioGroup
                                    label={t('regimeTrabalho')}
                                    options={[
                                        { value: 'full-time', label: t('integral'), description: t('integral') },
                                        { value: 'part-time', label: t('meioPeriodo'), description: t('meioPeriodo') },
                                        { value: 'hourly', label: t('temporarioRegime'), description: t('temporarioRegime') },
                                    ]}
                            ...
                            <div style={sectionStyle(isMobile)}>
                                ...
                                <RadioGroup
                                    label={t('pcdSim')}
                                    options={[
                                        { value: 'no', label: t('nao'), description: t('pcdNao') },
                                        { value: 'exclusive', label: t('pcdSim'), description: t('pcdSim') },
                                        { value: 'inclusive', label: t('sim'), description: t('pcdSim') },
                                    ]}
                            ...
                            <div style={sectionStyle(isMobile)}>
                                ...
                                <RadioGroup
                                    label={t('modeloTrabalho')}
                                    options={[
                                        { value: 'remote', label: t('remoto'), icon: <MapPin size={18} />, description: t('remoto') },
                                        { value: 'hybrid', label: t('hibrido'), icon: <Building2 size={18} />, description: t('hibrido') },
                                        { value: 'onsite', label: t('presencial'), icon: <Building2 size={18} />, description: t('presencial') },
                                    ]}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.3s ease-out' }}>
                            ...
                            <div style={sectionStyle(isMobile)}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                                    ...
                                    <div>
                                        <h2 style={{ color: 'var(--text-main)', fontSize: '20px', fontWeight: 700, margin: 0 }}>
                                            {t('responsabilidadesVaga')} *
                                        </h2>
                                        <p style={{ color: 'var(--text-muted)', fontSize: '13px', margin: '2px 0 0' }}>
                                            {t('descrevaResponsabilidades')}
                                        </p>
                                    </div>
                                </div>
                                <textarea
                            ...
                            <div style={sectionStyle(isMobile)}>
                                ...
                                <textarea
                                    ...
                                    placeholder={t('descrevaDiferenciais')}
                                    rows={5}
                                    style={{ ...inputStyle, resize: 'vertical', boxSizing: 'border-box' }}
                                    onFocus={(e) => {
                                        e.target.style.borderColor = 'var(--primary)';
                            ...
                            <div style={sectionStyle(isMobile)}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                                    ...
                                    <div>
                                        <h2 style={{ color: 'var(--text-main)', fontSize: '20px', fontWeight: 700, margin: 0 }}>
                                            {t('informacoesAdicionais')}
                                        </h2>
                                        <p style={{ color: 'var(--text-muted)', fontSize: '13px', margin: '2px 0 0' }}>
                                            {t('descrevaInfoAdicionais')}
                                        </p>
                                    </div>
                                </div>
                                <textarea
                                    ...
                                    placeholder={t('descrevaInfoAdicionais')}
                                    rows={6}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.3s ease-out' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center' }}>
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: isMobile ? '100%' : 'auto' }}>
                                                        <div style={{ display: 'flex', background: 'var(--bg-main)', padding: '4px', borderRadius: '8px', border: '1px solid var(--border)', flex: isMobile ? 1 : 'none', minWidth: 0 }}>
                                                            {[
                                                                { value: 'text', label: t('texto'), icon: <Type size={14} /> },
                                                                { value: 'paragraph', label: t('paragrafo'), icon: <List size={14} /> },
                                                                { value: 'choice', label: t('escolha'), icon: <CheckCircle2 size={14} /> }
                                                            ].map(opt => (
                                                                <button
                                                                    key={opt.value}
                                                                    type="button"
                                                ...
                                                {index > 0 && q.id !== '__salary_expectation__' && formData.customQuestions.slice(0, index).some(prevQ => prevQ.type === 'choice' && prevQ.options && prevQ.options.length > 0) && (
                                                    <div style={{ marginTop: '4px', padding: isMobile ? '12px' : '16px', background: 'rgba(99, 102, 241, 0.05)', borderRadius: '10px', border: '1px solid rgba(99, 102, 241, 0.2)' }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: q.logic ? '12px' : '0' }}>
                                                            ...
                                                            <span style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 600 }}>{t('configAvancadas')}</span>
                                                            {!q.logic ? (
                                                                <button 
                                                                    type="button"
                                                        ...
                                                        {q.logic && (
                                                            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center' }}>
                                                                <span style={{ color: 'var(--text-muted)', fontSize: '13px' }}>{t('dadosPrincipais')}</span>
                                                                <select
                                                                ...
                                                                >
                                                                    <option value="">{t('selecioneArea')}...</option>
                                                                    {formData.customQuestions.slice(0, index).filter(pq => pq.type === 'choice').map(pq => (
                                                                        <option key={pq.id} value={pq.id}>{pq.label || `${t('perguntasPersonalizadas')} ${formData.customQuestions.indexOf(pq) + 1}`}</option>
                                                                    ))}
                                                                </select>
                                                                <span style={{ color: 'var(--text-muted)', fontSize: '13px' }}>{t('local')}</span>
                                                                <select
                                                                ...
                                                                >
                                                                    <option value="">{t('localizacao')}...</option>
                                                            ...
                                                            {q.options?.map((opt, idx) => (
                                                                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        placeholder={`${t('adicionarOpcao')} ${idx + 1}`}
                                                        ...
                                                        <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid var(--border)' }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: q.hasComplementary ? '12px' : '0' }}>
                                                                ...
                                                                <label htmlFor={`comp-${q.id}`} style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                                                                    {t('informacoesAdicionais')} ({t('opcional')})
                                                                </label>
                                                            </div>
                                                            ...
                                                            {q.hasComplementary && (
                                                                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px', animation: 'dashFadeUp 0.3s ease-out' }}>
                                                                    <div>
                                                                        <label style={{ display: 'block', color: 'var(--text-muted)', fontSize: '12px', marginBottom: '6px' }}>{t('dadosPrincipais')}:</label>
                                                                        <select 
                                                                        ...
                                                                        >
                                                                            <option value="">{t('visaoGeral')}</option>
                                                                            {q.options?.map(opt => (
                                                                                <option key={opt} value={opt}>{opt}</option>
                                                                            ))}
                                                                        </select>
                                                                    ...
                                                                    <div>
                                                                        <label style={{ display: 'block', color: 'var(--text-muted)', fontSize: '12px', marginBottom: '6px' }}>{t('nomeEmpresa')}</label>
                                                                        <input 
                                                                            type="text"
                            ...
                            <div style={{ ...navContainerStyle(isMobile) }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '12px', width: isMobile ? '100%' : 'auto' }}>
                                    ...
                                    >
                                        <Save size={16} />
                                        {saving ? t('saving') : (isEditMode ? t('editarVaga') : t('salvar'))}
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ position: 'relative', zIndex: 1 }}>
                            ...
                            <div style={{
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                                    {[
                                        { name: t('triagem'), color: '#6366f1' },
                                        { name: t('entrevista'), color: '#0ea5e9' },
                                        { name: t('proposta'), color: '#f59e0b' },
                                        { name: t('aprovado'), color: '#22c55e' },
                                        { name: t('reprovado'), color: '#ef4444' }
                                    ].map((col, idx) => (
                                        <span key={idx} style={{
                                            display: 'inline-flex',
                                            alignItems: 'center',
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                ...
                                >
                                    {creatingPipeline ? t('saving') : t('criar')}
                                </button>
