...
export const AddCandidateModal = ({ isOpen, onClose, onSuccess, onViewCandidate }: AddCandidateModalProps) => {
  ...
  const validate = (): boolean => {
    ...
    if (!name.trim()) {
      errors.name = t('nomeObrigatorio');
    }

    if (email.trim()) {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) {
        errors.email = t('emailInvalido');
      }
    }
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    if (!allowedExtensions.includes(fileExt)) {
      toast.error(t('formatosSuportados'));
      return;
    }
    ...
    try {
      ...
      if (!extractedText || extractedText.trim().length < 50) {
        throw new Error(t('erroExtrairTexto'));
      }

      setUploadProgress(75);

      // Extrair dados com a função dedicada (sem scoring)
      const extractionResult = await extractCandidateData(extractedText, undefined, lang);
      ...
      toast.success(t('curriculoAnalisadoSucesso'));
    } catch (err) {
      console.error('Error processing CV:', err);
      const message = err instanceof Error ? err.message : t('erroProcessarCurriculo');
      setError(message);
      setUploadState('error');
      toast.error(`${t('erroLabel')} ${message}`);
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      onSuccess();
      onClose();
      toast.success(t('candidatoAdicionadoSucesso'));
    } catch (err) {
      const message = err instanceof Error ? err.message : t('erroAdicionarCandidato');
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t('adicionarCandidatoTitulo')}>
      <div style={{
        overflowY: 'auto',
        ...
        <form onSubmit={handleSubmit} style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {duplicateCandidate.phone && <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 2 }}>{t('phone')}: {duplicateCandidate.phone}</p>}
                  {duplicateCandidate.location && <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 8 }}>{t('localizacao')}: {duplicateCandidate.location}</p>}
                  <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 12 }}>
                    {t('candidatoDuplicadoMsg')}
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    ...
                    {t('visualizarCandidatoExistente')}
                  </button>
                </div>
              </div>
            </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('fullName')} *</span>
                  </div>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder={t('exNome')}
                    style={{
                      width: '100%',
                      background: 'transparent',
                      border: 'none',
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('email')}</span>
                  </div>
                  <input
                    type="email"
                    value={email}
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('phone')}</span>
                  </div>
                  <input
                    type="tel"
                    value={phone}
                    onChange={(e) => setPhone(e.target.value)}
                    placeholder={t('exTelefone')}
                    style={{
                      width: '100%',
                      background: 'transparent',
                      border: 'none',
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('localizacao')}</span>
                  </div>
                  <input
                    type="text"
                    value={location}
                    onChange={(e) => setLocation(e.target.value)}
                    placeholder={t('exLocalizacao')}
                    style={{
                      width: '100%',
                      background: 'transparent',
                      border: 'none',
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('idade')}</span>
                  </div>
                  <input
                    type="number"
                    value={age}
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('habilidades')}</span>
                </div>
                <textarea
                  value={skills}
                  onChange={(e) => setSkills(e.target.value)}
                  placeholder={t('exHabilidades')}
                  rows={3}
                  style={{
                    width: '100%',
                    background: 'transparent',
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <Briefcase style={{ width: 14, height: 14, color: 'var(--text-dim)' }} />
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('experiencia')}</span>
                </div>
                <textarea
                  value={experience}
                  onChange={(e) => setExperience(e.target.value)}
                  placeholder={t('exExperiencia')}
                  rows={3}
                  style={{
                    width: '100%',
                    background: 'transparent',
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{t('educacao')}</span>
                </div>
                <textarea
                  value={education}
                  onChange={(e) => setEducation(e.target.value)}
                  placeholder={t('exEducacao')}
                  rows={3}
                  style={{
                    width: '100%',
                    background: 'transparent',
          ...
          <div style={{
            ...
            <button
              ...
              title={duplicateCandidate ? t('candidatoDuplicadoTitle') : ''}
            >
              {loading && <Loader2 style={{ width: 14, height: 14, animation: 'acm-spin 1s linear infinite' }} />}
              {loading ? t('saving') : (uploadState === 'success' ? t('salvarCandidato') : t('adicionarCandidatoTitulo'))}
            </button>
          </div>
        </form>
      </div>
