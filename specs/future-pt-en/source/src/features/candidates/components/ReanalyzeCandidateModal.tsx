...
async function downloadResume(url: string, fileName: string, errorMsg: string): Promise<File> {
    let path = url;
    ...
    if (!data?.signedUrl) throw new Error(errorMsg);
...
export function ReanalyzeCandidateModal({ candidate, organizationId, userId, onClose, onSuccess }: Props) {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                toast.error(t('erroCarregarVagas'));
            } finally {
    ...
    async function runAnalysis() {
        ...
        try {
            ...
            customQuestions.forEach(q => {
                formAnswers[q.id] = `[${q.label}] ${t('naoRespondidoReanalise')}`;
            });
            ...
            if (candidate.resume_url) {
                const resumeFile = await downloadResume(
                    ...
                    'curriculo.pdf',
                    t('falhaGerarLinkDownload')
                );
                result = await analyzeJobApplication(resumeFile, vaga.title, jobDesc, formAnswers);
            }

            if (!result) {
                throw new Error(t('naoFoiPossivelAnalisar'));
            }

            setAiResult(result);
            setStep('preview');
        } catch (err) {
            const msg = err instanceof Error ? err.message : t('erroAoAnalisar');
            setError(msg);
            toast.error(msg);
        } finally {
            setAnalyzing(false);
    ...
    async function confirmSave() {
        ...
        try {
                ...
                if (pipe) {
                    ...
                    if (otherCard) {
                        const { data: otherPipe } = await supabase
                            ...
                            .single();
                        setCurrentPipelineName(otherPipe?.name || t('outroPipeline'));
                        ...
                        toast.success(`${t('analiseSalvaPipeline')} "${pipe.name}".`);
                        setStep('done');
                    }
                    return;
                }
            ...
            toast.success(`${t('analiseSalvaVaga')} "${vaga.title}"`);
            setStep('done');
        } catch (err) {
            const msg = err instanceof Error ? err.message : t('erroSalvarAnalise');
            setError(msg);
            toast.error(msg);
            setStep('preview');
        }
        ...
        try {
            ...
            if (addToPipeline) {
                ...
                if (firstColumnId) {
                    ...
                    toast.success(`${t('candidatoAdicionadoPipeline')} "${pipelineName}"`);
                }
            }

            const msg = addToPipeline
                ? hasOtherPipeline
                    ? `${t('analiseSalvaCandidatoMovido')} "${pipelineName}"`
                    : `${t('analiseSalvaCandidatoAdicionado')} "${pipelineName}"`
                : `${t('analiseSalvaParaVaga')} "${vaga.title}"`;
            toast.success(msg);
            setStep('done');
        } catch (err) {
            const msg = err instanceof Error ? err.message : t('erroGerenciarPipeline');
            toast.error(msg);
            setStep('done');
        }
    }
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: step === 'preview' ? '16px 24px' : 0 }}>
                        ...
                        <>
                            <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 12px' }}>
                                    ...
                                    <input value={vagaSearch} onChange={e => setVagaSearch(e.target.value)} placeholder={t('buscarVaga')}
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
                                {loadingVagas ? (
                                    <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                                        ...
                                        <p style={{ fontSize: 13, margin: 0 }}>{t('carregandoVagas')}</p>
                                    </div>
                                ) : filteredVagas.length === 0 ? (
                                    <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                                        <p style={{ fontSize: 13, margin: 0 }}>{t('nenhumaVaga')}</p>
                                    </div>
                                ) : (
                                        ...
                                        <div key={vaga.id} onClick={() => setSelectedVagaId(vaga.id)}
                                            ...
                                            <div>
                                                ...
                                                <span style={{ fontSize: 11, fontWeight: 600, marginTop: 4, display: 'inline-block', padding: '2px 8px', borderRadius: 6,
                                                    ...
                                                    {vaga.status === 'aberta' ? t('ativa') : t('invisivel')}
                                                </span>
                                            </div>
                    ...
                    {step === 'analyzing' && (
                        <div style={{ padding: '60px 24px', textAlign: 'center' }}>
                            ...
                            <p style={{ marginTop: 16, fontSize: 14, color: 'var(--text-dim)' }}>{t('analisando')}</p>
                            <p style={{ marginTop: 4, fontSize: 12, color: 'var(--text-muted)' }}>{t('iaAvaliando')}</p>
                        </div>
                    )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0', borderBottom: '1px solid var(--border)' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    ...
                                    <div>
                                        <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--text-main)' }}>{t('resultadoAnalise')}</p>
                            ...
                            {aiResult.summary && (
                                <div>
                                    <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('resumo')}</p>
                            ...
                            {aiResult.strengths && aiResult.strengths.length > 0 && (
                                <div>
                                    <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: '#22c55e', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('pontosFortes')}</p>
                            ...
                            {aiResult.gaps && aiResult.gaps.length > 0 && (
                                <div>
                                    <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('pontosAtencao')}</p>
                                    <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
                            ...
                            {aiResult.experience && (
                                <div>
                                    <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('experiencia')}</p>
                    ...
                    {step === 'pipeline' && (
                        <div style={{ padding: '40px 24px', textAlign: 'center' }}>
                            ...
                            <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--text-main)' }}>
                                {hasOtherPipeline
                                    ? `${t('candidatoJaEm')} "${currentPipelineName}"`
                                    : t('vagaPossuiPipeline')}
                            </h3>
                            <p style={{ marginTop: 8, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.5 }}>
                                {hasOtherPipeline
                                    ? `${t('desejaRemoverEAdicionar')} "${candidate.name}" ${t('de')} "${currentPipelineName}" ${t('eAdicionar')} "${pipelineName}"?`
                                    : `${t('desejaAdicionar')} "${candidate.name}" ${t('aoPipeline')} "${pipelineName}"?`}
                            </p>
                            ...
                            <div style={{ marginTop: 24, display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                                <button onClick={() => handlePipelineDecision(false)}
                                    ...
                                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#ef4444'; e.currentTarget.style.color = '#ef4444'; }}
                                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-dim)'; }}>
                                    {hasOtherPipeline ? t('soSalvarAnalise') : t('nao')}
                                </button>
                                <button onClick={() => handlePipelineDecision(true)}
                                    ...
                                    <Check size={18} />
                                    {hasOtherPipeline ? t('trocarPipeline') : t('simAdicionar')}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 'saving' && (
                        <div style={{ padding: '60px 24px', textAlign: 'center' }}>
                            ...
                            <p style={{ marginTop: 16, fontSize: 14, color: 'var(--text-dim)' }}>{t('salvandoAnalise')}</p>
                        </div>
                    )}
                    ...
                    {step === 'done' && (
                        <div style={{ padding: '60px 24px', textAlign: 'center' }}>
                            ...
                            <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: 'var(--text-main)' }}>{t('analiseSalvaSucesso')}</h3>
                ...
                {(step === 'select') && (
                    <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                        ...
                        <button onClick={runAnalysis} disabled={!selectedVagaId || analyzing}
                            ...
                            {analyzing ? <><Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> {t('analisando')}</> : <>{t('analisar')}</>}
                        </button>
                    </div>
                )}
