...
export function TalentTransferModal({ candidate, job, onClose, onSuccess }: TalentTransferModalProps) {
    ...
    const handleCreatePipeline = async () => {
        ...
        try {
            ...
            toast.success(t('pipelineColunasCriados'));
        } catch (err: unknown) {
            toast.error(t('erroCriarPipeline') + (err as Error).message);
        } finally {
    ...
    const handleLinkExisting = async () => {
        ...
        try {
            ...
            setHasPipeline(true);
            toast.success(t('pipelineVinculado'));
        } catch (err: unknown) {
            toast.error(t('erroVincularPipeline') + (err as Error).message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleTransfer = async (addToPipeline: boolean) => {
        ...
        try {
            ...
            if (jcError) {
                toast.error(t('erroVincularVaga') + jcError.message);
            }
            ...
            toast.success(t('candidatoMovidoBanco'));
            ...
            toast.error(t('errosTransferencia') + (err as Error).message);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div style={{ 
                ...
                <div style={{ padding: '24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: 'var(--text-main)' }}>{t('transferirParaTalento')}</h3>
                    </div>
                ...
                <div style={{ padding: '32px 24px' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ marginBottom: '24px' }}>
                                <div style={{ fontSize: '16px', color: 'var(--text-main)', fontWeight: 500, marginBottom: '8px' }}>
                                    {t('desejaMover')} <span style={{ color: 'var(--primary)', fontWeight: 700 }}>{candidate.name}</span> {t('paraBancoTalentos')}
                                </div>
                                <p style={{ fontSize: '14px', color: 'var(--text-dim)', margin: 0 }}>
                                    {t('candidatoDisponivel')}
                                </p>
                            </div>
                    ...
                    {step === 'pipeline' && (
                        <div>
                            {isBlacklisted && (
                                <div style={{ 
                                    ...
                                    <div style={{ flex: 1, textAlign: 'left' }}>
                                        <p style={{ fontSize: '13px', fontWeight: 700, color: '#ef4444', margin: '0 0 4px' }}>{t('candidatoBlacklist')}</p>
                                        <p style={{ fontSize: '12px', color: 'var(--text-dim)', margin: 0, lineHeight: '1.4' }}>
                                            {t('candidatoBlacklistMsg')}
                                        </p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div style={{ marginBottom: '24px', textAlign: 'center' }}>
                                ...
                                <h4 style={{ fontSize: '18px', fontWeight: 700, margin: '0 0 8px' }}>{t('integracaoPipeline')}</h4>
                                <p style={{ fontSize: '14px', color: 'var(--text-dim)', margin: 0 }}>
                                    {t('desejaIniciarTriagem')}
                                </p>
                            </div>
                            ...
                            {!isBlacklisted && !hasPipeline && hasPipeline !== null && (
                                <div style={{ 
                                    ...
                                    <div style={{ display: 'flex', gap: '12px', marginBottom: showSelection ? '16px' : '0' }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <p style={{ fontSize: '13px', fontWeight: 700, color: '#f59e0b', margin: '0 0 4px' }}>{t('pipelineNaoEncontrado')}</p>
                                            <p style={{ fontSize: '12px', color: 'var(--text-dim)', margin: '0 0 12px', lineHeight: '1.4' }}>
                                                {t('pipelineNaoEncontradoMsg')}
                                            </p>
                                    ...
                                    {showSelection && (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, animation: 'fadeIn 0.2s ease' }}>
                                            ...
                                            >
                                                <option value="">{t('selecionarOpcao')}</option>
                                                {allPipelines.map(p => (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                ...
                                >
                                    ...
                                    {t('adicionarBancoIniciarTriagem')}
                                </button>
                                <button
                    ...
                    {step === 'success' && (
                        <div style={{ textAlign: 'center', padding: '20px 0' }}>
                            ...
                            <h4 style={{ fontSize: '20px', fontWeight: 800, color: 'var(--text-main)', margin: '0 0 12px' }}>{t('sucesso')}</h4>
                            <p style={{ fontSize: '15px', color: 'var(--text-dim)', margin: '0 0 32px', lineHeight: 1.6 }}>
                                {t('candidatoBancoTalentos')}
                            </p>
                            <button
                                onClick={onClose}
                                style={{ 
