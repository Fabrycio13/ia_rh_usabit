...
const isMissing = (val: string | null | undefined): boolean => {
    ...
    return ['não informado', 'não informado.', 'n/a', 'desconhecido', '—', 'não identificado', 'not informed', 'unknown', 'not identified'].includes(v);
};
...
export const AnalysisProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const startAnalysis = async (files: File[], name: string, desc: string, mode: 'pdf' | 'excel') => {
        ...
        try {
            ...
            if (!session) throw new Error(t('sessaoExpirada'));
            ...
            if (jobError || !jobData) {
                const msg = jobError ? jobError.message : t('erroCriarRegistroVaga');
                throw new Error(`${t('erroCriarRegistroAnalise')} ${msg}`);
            }
                    ...
                    try {
                        ...
                        if (mode === 'pdf' && files[idx]) {
                            ...
                            if (uploadError) {
                                ...
                                toast.error(t('erroSalvarPdf').replace('{file}', file.name));
                                resumeUrl = null;
                            } else {
                        ...
                        const redFlagsPart = (typeof c.redFlags === 'string' && !['nenhuma identificada', 'nenhuma', 'n/a', 'none identified', 'none'].includes(c.redFlags.toLowerCase())) ? c.redFlags : '';
                        let combinedAttentionPoints = [gapsPart, redFlagsPart].filter(Boolean).join('\n');
                        if (!combinedAttentionPoints) combinedAttentionPoints = t('semPontosAtencao');
                        ...
                        setResult(prev => {
                            ...
                            const newSummary = `${t('analisandoResumido')} ${newCandidates.length} ${t('curriculosLower')}. (${bestCount} ${t('melhores')}, ${midCount} ${t('intermediarios')}, ${worstCount} ${t('piores')})`;
                        ...
                        if (existingId) {
                            ...
                            if (e4) {
                                ...
                                toast.error(`${t('erroAtualizarCandidato')} ${normalizedCandidate.name}: ${e4.message}`);
                            }
                            dbRecord = upd;
                            if (dbRecord) {
                            ...
                            if (e5) {
                                ...
                                toast.error(`${t('erroSalvarCandidato')} ${normalizedCandidate.name}: ${e5.message}`);
                            }
                            dbRecord = ins;
                            if (dbRecord) {
                        ...
                        toast.error(`${t('erroTratarCandidato')} ${idx + 1}: ${error.message || t('erroDesconhecido')}`);
                    }
                },
                (err, idx) => {
                    console.error(`[Analysis] Erro no currículo ${idx + 1}:`, err);
                    toast.error(`${t('erroNoCurriculo')} ${idx + 1}: ${err}`, { duration: 6000, id: `err-${idx}` });
                }
            );
            ...
            setResult(prev => {
                ...
                let summary = `${t('analiseConcluidaPara')} "${name}". ${t('totalResumido')} ${finalCandidates.length}. (${bestCount} ${t('melhores')}, ${midCount} ${t('intermediarios')}, ${worstCount} ${t('piores')}).`;
                if (analysisErrors.length > 0) summary += ` (⚠️ ${analysisErrors.length} ${t('falhas')})`;

                return { summary, candidates: finalCandidates };
            });

            setAnalyzing(false);
            toast.success(`${t('analiseConcluida')} ${name}`);
            ...
            if (profile.notificationsEnabled && Notification.permission === 'granted') {
                try {
                    const notification = new Notification(t('analiseConcluidaNotif'), {
                        body: t('analiseConcluidaBody').replace('{name}', name),
                        icon: '/favicon.ico',
