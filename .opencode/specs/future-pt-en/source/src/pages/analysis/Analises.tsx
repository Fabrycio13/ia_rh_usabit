  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        toast.error(t('erroCarregarAnalise') + ': ' + ((err as Error).message ?? t('erroDesconhecido')));
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="text-[var(--text-main)]">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--text-main)]">{job?.name ?? t('analise')}</h1>
            {job && <p className="text-[var(--text-dim)] text-xs mt-0.5">{new Date(job.created_at).toLocaleDateString('pt-BR')} {t('as')} {new Date(job.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p>}
          </div>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <span className="bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text-dim)] rounded-xl px-3 py-1.5">{candidates.length} {t('candidatos')}</span>
          <span style={{ background: 'var(--success-bg)', borderColor: 'var(--success)', color: 'var(--success)' }} className="border rounded-xl px-3 py-1.5">{best.length} {t('aprovados')}</span>
        </div>
      </div>
      ...
      {candidates.length === 0 ? (
        <div className="bg-[var(--bg-card)] border border-white/5 rounded-2xl p-16 text-center">
          <p className="text-slate-400 text-lg mb-2">{t('nenhumCandidatoNestaAnalise')}</p>
          <p className="text-slate-500 text-sm">{t('candidatosNaoSalvos')}</p>
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-main)' }}>
                {[t('rank'), t('nome'), t('idade'), t('localizacao'), t('genero'), t('score'), ...(hasPermission(profile.user_role, 'chat') ? [t('chat')] : []), t('acoes')].map(h => (
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'center' }}>
                        {c.conversations?.length ? (
                          <div title={t('chatAtivo')}>
                            <Phone size={16} color="#22c55e" fill="#22c55e22" />
                          </div>
                        ) : (
                          <div title={t('chatInativo')}>
...
export const Analises = ({ hideHeader }: { hideHeader?: boolean }) => {
  ...
  fetchAnalisesRef.current = async function fetchAnalises(userId: string) {
    try {
      ...
      setError(t('erroCarregarAnalises'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDeleteJob(e: React.MouseEvent, jId: string, jName: string) {
    e.stopPropagation();
    if (!window.confirm(t('confirmarExclusaoAnalise').replace('{name}', jName))) return;
    try {
      ...
      toast.success(t('analiseExcluida'));
      setJobs(prev => prev.filter(j => j.id !== jId));
      if (profile.userId) {
        logActivity(profile.userId, t('excluiuAnalise').replace('{name}', jName));
      }
    } catch (err) {
      console.error('Erro ao excluir análise:', err);
      toast.error(t('erroExcluirAnalise'));
    }
  }
    ...
    return (
      <div className="flex items-center justify-center h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-[#6366f1] border-t-transparent rounded-full animate-spin" />
          <p className="text-slate-400 text-sm">{t('carregandoAnalises')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center h-[60vh]">
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 text-center max-w-md">
          <p className="text-red-400 font-semibold mb-2">{t('erroCarregar')}</p>
    ...
    <div className="text-[var(--text-main)]">
      ...
      <div className="flex justify-between items-start mb-8">
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '8px' }}>
              ...
              <h1 style={{ fontSize: '32px', fontWeight: 700, color: 'var(--text-main)', margin: 0 }}>
                {t('iaAnaliseCurriculos')}
              </h1>
            </div>
            <p style={{ color: 'var(--text-muted)', fontSize: '14px', margin: 0 }}>
              {t('gerencieCurriculos')}
            </p>
          </div>
        )}
        <div className="flex items-center gap-4" style={{ marginLeft: hideHeader ? 'auto' : undefined, flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center', width: isMobile ? '100%' : 'auto' }}>
          ...
          >
            <Plus className="w-4 h-4" /> {t('novaAnalise')}
          </button>
        </div>
      </div>
      ...
      {recent.length > 0 && (
        <>
          <p className="text-xs text-[var(--text-dim)] uppercase tracking-widest mb-4">{t('acessadosRecentemente')}</p>
      ...
      <div style={{ background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 18px', marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center' }}>
        <span style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600, marginRight: 4 }}>{t('filtrarPeriodo')}</span>
      ...
      {/* Tabela */}
      <p className="text-xs text-[var(--text-dim)] uppercase tracking-widest mb-4">{t('ultimasAnalises')}</p>

      {jobs.length === 0 ? (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-16 text-center">
          <p className="text-[var(--text-muted)] text-lg mb-2">{t('nenhumaAnalise')}</p>
          <p className="text-[var(--text-dim)] text-sm mb-6">{t('cliqueNovaAnalise')}</p>
          <button
            onClick={() => navigate('/analise/nova?new=true')}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {paginated.map(j => (
            <div key={j.id} onClick={() => navigate(`/analise/${j.id}`)} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <button onClick={(e) => handleDeleteJob(e, j.id, j.name)} className="text-slate-500 hover:text-red-500 transition-colors" title={t('excluir')} style={{ minWidth: 44, minHeight: 44, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Trash2 size={16} />
                </button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12, color: 'var(--text-dim)' }}>
                <div><span style={{ color: 'var(--text-dim)', opacity: 0.7 }}>{t('avaliados')}:</span> <span style={{ color: 'var(--text-main)', fontWeight: 600 }}>{j.totalCandidates}</span></div>
                <div><span style={{ color: 'var(--text-dim)', opacity: 0.7 }}>{t('top')}:</span> <span style={{ color: 'var(--text-main)', fontWeight: 600 }}>{j.topCandidates}</span></div>
                <div><span style={{ color: 'var(--text-dim)', opacity: 0.7 }}>{t('idade')}:</span> {j.filters?.age || '—'}</div>
                <div><span style={{ color: 'var(--text-dim)', opacity: 0.7 }}>{t('genero')}:</span> {j.filters?.gender || '—'}</div>
                <div style={{ gridColumn: '1 / -1' }}><span style={{ color: 'var(--text-dim)', opacity: 0.7 }}>{t('local')}:</span> {j.filters?.location || '—'}</div>
              </div>
          ...
          {totalPages > 1 && (
            <div style={{ background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border)', padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexDirection: 'column', gap: 10 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>{t('pagina')} {page} {t('de')} {totalPages} · {filtered.length} {t('analisesEncontradas')}</span>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
                <button onClick={() => goTo(page - 1)} disabled={page === 1}
                  ...
                  <ChevronLeft style={{ width: 15, height: 15 }} /> {t('anterior')}
                </button>
                <button onClick={() => goTo(page + 1)} disabled={page === totalPages}
                  ...
                  {t('proximo')} <ChevronRight style={{ width: 15, height: 15 }} />
                </button>
              </div>
            </div>
          )}
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-2xl">
          <table className="w-full text-sm">
            <thead style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
              <tr>
                <th className="px-6 py-4 text-left font-medium text-[var(--text-dim)]">{t('nomeDaVaga')}</th>
                <th className="px-6 py-4 text-center font-medium text-[var(--text-dim)]">{t('candidatosAvaliados')}</th>
                <th className="px-6 py-4 text-center font-medium text-[var(--text-dim)]">{t('melhoresCandidatos')}</th>
                <th className="px-6 py-4 text-center font-medium text-[var(--text-dim)]">{t('faixaEtaria')}</th>
                <th className="px-6 py-4 text-center font-medium text-[var(--text-dim)]">{t('genero')}</th>
                <th className="px-6 py-4 text-center font-medium text-[var(--text-dim)]">{t('localidade')}</th>
                <th className="px-6 py-4 text-center font-medium text-[var(--text-dim)]">{t('ultimaAtualizacao')}</th>
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderTop: '1px solid var(--border)', background: 'var(--bg-card)' }}>
              <span style={{ fontSize: 13, color: '#64748b' }}>{t('pagina')} {page} {t('de')} {totalPages} · {filtered.length} {t('analisesEncontradas')}</span>
