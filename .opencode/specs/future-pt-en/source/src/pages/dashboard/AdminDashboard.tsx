...
export const AdminDashboard = () => {
                ...
                .reduce((acc: {id: string, name: string}[], u) => {
                    if (!acc.find(o => o.id === u.organization_id)) {
                        acc.push({ 
                            ...
                            name: u.organization_name || t('novaOrganizacao')
                        });
                    }
                    return acc;
                }, [])
    ...
    const handleToggleVagaAccess = async (convidadoUserId: string, vagaId: string, hasAccess: boolean) => {
        if (hasAccess) {
            ...
            if (error) { toast.error(t('erroRemoverAcesso')); return; }
            ...
            if (error) { toast.error(t('erroAdicionarAcesso')); return; }
    ...
    const handleCreateUser = async () => {
        if (!newUser.name || !newUser.email) {
            toast.error(t('preenchaTodosCampos'));
            return;
        }
        ...
        if (!canCreate(userRole, newUser.user_role)) {
            const allowed = userRole === 'owner' ? t('perfilAdministrador') :
                            userRole === 'administrador' ? t('perfisSupervisorRhConvidado') :
                            t('perfisRhConvidado');
            toast.error(`${t('seuPerfilSoPodeCriar')} ${allowed}`);
            return;
        }
        ...
        try {
            ...
            if (fnError) {
                ...
                toast.error(`${t('erroCriarConvite')} ${detail.substring(0, 300)}`);
                setCreatingUser(false);
                return;
            }
            const userId = fnData?.userId;
            if (!userId) {
                toast.error(`${t('erroUserIdNaoRetornado')} ${JSON.stringify(fnData).substring(0, 2000)}`);
                setCreatingUser(false);
                return;
            }
            toast.success(`${t('conviteEnviadoPara')} ${newUser.name}!`);
            ...
            toast.error(`${t('erroInesperado')} ${(err as Error).message}`);
        }
        setCreatingUser(false);
    };
    ...
    const handleToggleOrgStatus = async (orgId: string, currentStatus: string) => {
        if (!orgId) {
            toast.error(t('erroDesativarSemOrg'));
            return;
        }
        const newStatus = currentStatus === 'active' ? 'inactive' : 'active';
        const { error } = await supabase
        ...
        if (error) {
            toast.error(`${t('erroAtualizarOrganizacao')} ${error.message}`);
            return;
        }
        setUsers(prev => prev.map(u => u.organization_id === orgId ? { ...u, status: newStatus } : u));
        toast.success(`${t('organizacao')} ${newStatus === 'active' ? t('ativada') : t('desativada')} ${t('comSucesso')}`);
        logActivity(profile.userId, `${newStatus === 'active' ? t('ativouOrg') : t('desativouOrg')}`, { organization_id: orgId });
    };
    ...
    const handleResendInvite = async (targetUser: { id: string; name?: string; email: string }) => {
        ...
        if (error) {
            toast.error(t('erroReenviarConvite'));
            logActivity(profile.userId, t('erroReenviarConvite'), { email: targetUser.email, error: error.message });
            return;
        }
        toast.success(`${t('conviteReenviadoPara')} ${targetUser.name || targetUser.email}`);
        logActivity(profile.userId, t('reenviouConvite'), { email: targetUser.email });
    };
    ...
    return (
        <div style={{ width: '100%', margin: '0' }}>
            ...
            <div style={{ marginBottom: '32px' }}>
                ...
                {userRole === 'owner' && (
                    <div style={{ marginTop: '20px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
                        {!isMobile && <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-dim)', whiteSpace: 'nowrap', flexShrink: 0 }}>{t('filtrarDashboardPorOrg')}:</span>}
                            ...
                            <div className="cs-trigger" onClick={() => setIsOrgOpen(!isOrgOpen)}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, overflow: 'hidden' }}>
                                    ...
                                    <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                        {organizations.find(o => o.id === selectedOrgId)?.name || t('todasOrganizacoesVisaoGlobal')}
                                    </span>
                                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: isMobile ? '12px' : '20px', marginBottom: isMobile ? '20px' : '32px' }}>
                {[
                    { label: t('totalUsuarios'), value: stats.total, icon: Users, color: '#3b82f6' },
                    { label: t('usuariosAtivos'), value: stats.active, icon: UserCheck, color: '#10b981' },
                    { label: t('usuariosInativos'), value: stats.inactive, icon: UserX, color: '#ef4444' },
                ].map((s, i) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 340px', gap: isMobile ? '12px' : '20px', marginBottom: '32px' }}>
                ...
                <div style={{ background: 'var(--bg-card)', padding: '14px', borderRadius: '20px', border: '1px solid var(--border)', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ marginBottom: 12 }}>
                        <p style={{ color: 'var(--text-dim)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{t('filtro')}</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600, whiteSpace: 'nowrap' }}>{t('periodo')}:</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, opacity: 0.8 }}>{t('de')}</span>
                            <DatePicker compact value={rangeStart || ''} onChange={val => setRangeStart(val || null)} />
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 600, opacity: 0.8 }}>{t('ate')}</span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-main)', margin: 0 }}>
                                {activeStart 
                                    ? `${t('atividades')} ${fmtDate(activeStart)}${activeEnd ? ` → ${fmtDate(activeEnd)}` : ''}`
                                    : t('atividadesUltimos7Dias')
                                }
                            </h3>
                        </div>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height={isMobile ? 180 : 300} debounce={50}>
                            <AreaChart data={analysisData}>
                                ...
                                <RechartsTooltip 
                                    ...
                                    formatter={(value: number | undefined) => [value || 0, t('atividades')]}
                ...
                <div style={{ background: 'var(--bg-card)', padding: '20px', borderRadius: '20px', border: '1px solid var(--border)', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                        <div>
                            <p style={{ color: 'var(--text-dim)', fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '2px' }}>{t('filtro')}</p>
                    ...
                    <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                            ...
                            <span style={{ color: 'var(--text-dim)', fontSize: '11px' }}>{t('diaComVagaCriada')}</span>
                        </div>
                        {activeStart ? (
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <span style={{ color: 'var(--text-dim)', fontSize: '11px' }}>
                                    {activeEnd && activeEnd !== activeStart
                                        ...
                                        : <><b style={{ color: 'var(--primary)' }}>{fmtDate(activeStart)}</b>{!rangeEnd ? <span style={{ color: '#f59e0b' }}> {t('cliqueParaFinalizar')}</span> : ''}</>
                                    }
                                </span>
                                <button onClick={clearRange} style={{ fontSize: '10px', color: '#ef4444', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '6px', padding: '2px 8px', cursor: 'pointer', fontWeight: 600 }}>{t('limpar')}</button>
                            </div>
                        ) : (
                            <span style={{ color: 'var(--text-dim)', fontSize: '11px' }}>{t('cliqueDiaOuPeriodo')}</span>
                        )}
                    </div>
                </div>
                )}
            ...
            <div style={{ background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 18px', marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600 }}>{t('cargo')}:</span>
                        <div className="cs-container" ref={roleRef} style={{ width: isMobile ? '100%' : '160px' }}>
                            <div className="cs-trigger" onClick={() => setIsRoleOpen(!isRoleOpen)}>
                                <span>{roleFilter === '' ? t('todos') : roleFilter.toUpperCase()}</span>
                                <ChevronDown size={14} style={{ transition: 'transform 0.2s', transform: isRoleOpen ? 'rotate(180deg)' : 'none' }} />
                            </div>
                            {isRoleOpen && (
                                <div className="cs-dropdown" style={{ ...(isMobile ? { left: 'auto', right: 0, maxWidth: 'calc(100vw - 32px)' } : {}) }}>
                                    <div className={`cs-item ${roleFilter === '' ? 'active' : ''}`} onClick={() => { setRoleFilter(''); setIsRoleOpen(false); }}>{t('todos')}</div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600 }}>{t('status')}:</span>
                ...
                {(roleFilter || statusFilter || search) && (
                    ...
                    >
                        <X style={{ width: 12, height: 12 }} /> {t('limparFiltros')}
                    </button>
                )}
            </div>
                                ...
                                <>
                                    {orgEntries.length > 1 && (
                                        <div style={{ padding: '10px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end' }}>
                                            ...
                                            >
                                                {allExpanded ? t('recolherTodasOrg') : t('expandirTodasOrg')}
                                            </button>
                                        </div>
                                    )}
                                    {orgEntries.map(org => {
                                                ...
                                                >
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
                                                        ...
                                                        <div style={{ minWidth: 0 }}>
                                                            ...
                                                            <p style={{ color: 'var(--text-dim)', fontSize: '11px', margin: '2px 0 0' }}>
                                                                {totalMembros} {totalMembros === 1 ? t('membro') : t('membros')} {adminUser ? `• ${t('admin')}: ${adminUser.name || adminUser.email.split('@')[0]}` : ''}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }} onClick={e => e.stopPropagation()}>
                                                        ...
                                                        <span style={{ fontSize: 12, color: orgStatus === 'active' ? '#10b981' : orgStatus === 'pending' ? '#f59e0b' : '#ef4444', fontWeight: 600 }}>
                                                            {orgStatus === 'active' ? t('ativa') : orgStatus === 'pending' ? t('pendente') : t('inativa')}
                                                        </span>
                                                        {org.id !== 'sem-org' && (
                                                            <button
                                                            ...
                                                            >
                                                                ...
                                                                {orgStatus === 'active' ? t('desativar') : t('ativar')}
                                                            </button>
                                                        )}
                                                    </div>
                                                </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', tableLayout: isMobile ? 'auto' : 'fixed' }}>
                        ...
                        <thead>
                            <tr style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
                                <th style={{ padding: isMobile ? '12px 6px 12px 8px' : '14px 16px', fontSize: isMobile ? '10px' : '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{t('usuario')}</th>
                                <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', display: isMobile ? 'none' : 'table-cell' }}>{t('organizacao')}</th>
                                <th style={{ padding: isMobile ? '12px 2px' : '14px 16px', fontSize: isMobile ? '10px' : '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', textAlign: 'center' }}>{t('cargo')}</th>
                                <th style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center', display: isMobile ? 'none' : 'table-cell' }}>{t('status')}</th>
                                <th style={{ padding: isMobile ? '12px 4px' : '14px 16px', fontSize: isMobile ? '10px' : '11px', color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', textAlign: 'center' }}>{t('acoes')}</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr key={user.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.2s' }}>
                                            ...
                                            <div style={{ minWidth: 0, position: 'relative', cursor: 'pointer' }} onClick={() => setDetailUserId(user.id)}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                    <p style={{ color: 'var(--text-main)', fontWeight: 500, fontSize: 13, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        {(() => { const n = user.name || user.email.split('@')[0] || t('usuario'); const m = isMobile ? 12 : 999; return n.length > m ? n.substring(0, m) + '…' : n; })()}
                                                    </p>
                                                    {isMobile && (
                                    ...
                                    <td style={{ padding: isMobile ? '10px 8px' : '16px', display: isMobile ? 'none' : 'table-cell' }}>
                                        <p style={{ color: 'var(--text-main)', fontSize: 13, margin: 0, fontWeight: 500 }}>
                                            {user.organization_name || t('semOrganizacao')}
                                        </p>
                                    </td>
                                    ...
                                    <td style={{ padding: isMobile ? '10px 8px' : '16px', display: isMobile ? 'none' : 'table-cell' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                            ...
                                            <span style={{ fontSize: '13px', color: user.status === 'active' ? '#10b981' : user.status === 'pending' ? '#f59e0b' : '#ef4444' }}>
                                                {user.status === 'active' ? t('ativo') : user.status === 'pending' ? t('pendente') : t('inativo')}
                                            </span>
                                        </div>
                                    </td>
                                    <td style={{ padding: isMobile ? '10px 8px' : '10px 8px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'center', gap: isMobile ? '6px' : '6px' }}>
                                            <button
                                                ...
                                                title={user.status === 'active' ? t('desativar') : t('ativar')}
                                                style={{
                                            ...
                                            >
                                                ...
                                                {isMobile ? '' : (user.status === 'active' ? t('desativar') : user.status === 'pending' ? t('ativar') : t('ativar'))}
                                            </button>
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleResendInvite({ id: user.id, name: user.name, email: user.email }); }}
                                                title={t('reenviarConvite')}
                                                style={{
            ...
            {showCreateModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '16px' }}>
                    <div style={{ background: 'var(--bg-card)', borderRadius: '20px', padding: isMobile ? '20px' : '28px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                            <h2 style={{ color: 'var(--text-main)', fontSize: '18px', fontWeight: 700, margin: 0 }}>{t('convidarUsuario')}</h2>
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            <label htmlFor="newuser-name" style={labelStyle}>{t('fullName')}</label>
                            <div style={fieldWrapStyle}>
                                <UserIcon style={iconFieldStyle} />
                                <input
                                    id="newuser-name"
                                    style={inputStyle}
                                    placeholder={t('nomeUsuarioPlaceholder')}
                                    value={newUser.name}
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            <label htmlFor="newuser-email" style={labelStyle}>{t('email')}</label>
                            <div style={fieldWrapStyle}>
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            <label style={labelStyle}>{t('perfilAcesso')}</label>
                        ...
                        >
                            ...
                            {creatingUser ? t('enviandoConvite') : t('enviarConvite')}
                        </button>
                    </div>
                </div>
            )}
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '16px' }}>
                    <div style={{ background: 'var(--bg-card)', borderRadius: '20px', padding: isMobile ? '20px' : '24px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                            <h2 style={{ color: 'var(--text-main)', fontSize: '18px', fontWeight: 700, margin: 0 }}>{t('permissaoVagas')}</h2>
            ...
            {detailUserId && (() => {
                ...
                return (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '16px' }}
                        ...
                        <div style={{ background: 'var(--bg-card)', borderRadius: '20px', padding: isMobile ? '20px' : '28px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                                <h2 style={{ color: 'var(--text-main)', fontSize: '18px', fontWeight: 700, margin: 0 }}>{t('dadosUsuario')}</h2>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                    <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('cargo')}</span>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                    <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('status')}</span>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                        ...
                                        <span style={{ fontSize: '13px', fontWeight: 500, color: du.status === 'active' ? '#10b981' : du.status === 'pending' ? '#f59e0b' : '#ef4444' }}>
                                            {du.status === 'active' ? t('ativo') : du.status === 'pending' ? t('pendente') : t('inativo')}
                                        </span>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                    <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('organizacao')}</span>
                                    <span style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 500 }}>{du.organization_name || t('semOrganizacao')}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                    <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('cadastro')}</span>
                                    <span style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 500 }}>
                                        {du.created_at ? new Date(du.created_at).toLocaleDateString('pt-BR') : '-'}
                                    </span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                    <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('tipoConta')}</span>
                                    <span style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 500, textTransform: 'capitalize' }}>
                                        {du.account_type || 'active'}
                                        {du.trial_ends_at ? ` (${t('expira')} ${new Date(du.trial_ends_at).toLocaleDateString('pt-BR')})` : ''}
                                    </span>
                                </div>
                                {du.notifications_enabled !== undefined && (
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                        <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('notificacoes')}</span>
                                        <span style={{ color: du.notifications_enabled ? '#10b981' : '#ef4444', fontSize: '13px', fontWeight: 500 }}>
                                            {du.notifications_enabled ? t('ativadas') : t('desativadas')}
                                        </span>
                                    </div>
                                )}
                                {du.onboarding_completed !== undefined && (
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-main)', borderRadius: '10px' }}>
                                        <span style={{ color: 'var(--text-dim)', fontSize: '12px', fontWeight: 600 }}>{t('onboarding')}</span>
                                        <span style={{ color: du.onboarding_completed ? '#10b981' : '#f59e0b', fontSize: '13px', fontWeight: 500 }}>
                                            {du.onboarding_completed ? t('concluido') : t('pendente')}
                                        </span>
                                    </div>
                                )}
                            </div>
...
style={{ padding: '8px 16px', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--bg-main)', color: du.status === 'active' ? '#ef4444' : du.status === 'pending' ? '#f59e0b' : '#10b981', fontSize: '12px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s' }}
                                                                >
                                                                    ...
                                                                    {du.status === 'active' ? t('desativar') : du.status === 'pending' ? t('ativar') : t('ativar')}
                                </button>
                                <button
