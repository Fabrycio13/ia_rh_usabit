...
export const SpontaneousApplication = () => {
    ...
    const totalSteps = 3;
    const stepLabels = [t('stepNome'), t('stepDados'), t('stepCurriculo')];
    ...
    useEffect(() => {
        const fetchOrgData = async () => {
            ...
            try {
                ...
                if (!response.ok) throw new Error(t('organizacaoNaoEncontrada'));
                const { orgInfo } = await response.json();
                setOrgName(orgInfo?.name || '');
            } catch {
                setError(t('organizacaoNaoEncontrada'));
            } finally {
                setLoading(false);
            }
        };
        ...
        return (
            <div style={{ minHeight: '100vh', background: '#0B1020', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
                ...
                <div style={{ textAlign: 'center', maxWidth: 500 }}>
                    ...
                    <h1 style={{ color: '#f1f5f9', fontSize: 28, fontWeight: 700, marginBottom: 12 }}>{error || t('paginaNaoEncontrada')}</h1>
            ...
            <div style={{
                ...
                <div style={{ textAlign: 'center', maxWidth: '480px', animation: 'fadeSlideUp 0.6s ease-out', position: 'relative', zIndex: 1 }}>
                    ...
                    <h2 style={{ fontSize: '32px', fontWeight: 700, marginBottom: '16px', color: '#fff' }}>
                        {t('curriculoCadastrado')}
                    </h2>
                    <p style={{ color: '#94a3b8', fontSize: '16px', lineHeight: '1.6', marginBottom: '32px' }}>
                        {t('recebemosCurriculo')}, <strong style={{ color: '#fff' }}>{formData.name.split(' ')[0]}</strong>!<br /><br />
                        {t('contatoOportunidade')} 🍀
                    </p>
                    <button
    ...
    const stepMessages: Record<number, { bubble: string }> = {
        0: {
            bubble: t('bubbleEsp0').replace('{orgName}', orgName)
        },
        1: {
            bubble: t('bubbleEsp1').replace('{name}', firstName)
        },
        2: {
            bubble: t('bubbleEsp2').replace('{name}', firstName)
        }
    };
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <input
                                        ...
                                        type="text" placeholder={t('seuNomeCompleto')}
                                        value={formData.name}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                                ...
                                <div style={{ display: 'flex', gap: '8px' }}>
                                        ...
                                        {countryOpen && (
                                            <div className="cs-dropdown-wizard">
                                                <div className="cs-search-wrapper">
                                                    <input autoFocus className="cs-search-input" placeholder={t('selecionePais')} value={countrySearch} onChange={e => setCountrySearch(e.target.value)} onClick={e => e.stopPropagation()} />
                                                </div>
                                    ...
                                    <div style={{ position: 'relative', flex: 1 }}>
                                        ...
                                        <input className="wizard-input" style={{ paddingLeft: 46 }} type="tel" placeholder={selectedCountry.code + ' ' + t('placeholderTelefone')} value={formData.phone} onChange={e => handlePhoneChange(e.target.value)} />
                                    </div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 10 }}>
                                    <div style={{ position: 'relative' }}>
                                        ...
                                        <input className="wizard-input" style={{ paddingLeft: 46 }} type="text" placeholder={t('placeholderIdade')} value={formData.age} onChange={e => { let val = e.target.value.replace(/\D/g, ''); if (val.length > 2) val = val.slice(0, 2); setFormData(p => ({ ...p, age: val })); }} />
                                    </div>
                                    <div style={{ position: 'relative' }} ref={genderRef}>
                                        <div className={`cs-trigger-wizard ${genderOpen ? 'open' : ''}`} onClick={() => setGenderOpen(!genderOpen)}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                ...
                                                <span style={{ color: formData.gender ? '#f1f5f9' : '#475569' }}>{formData.gender || t('selecioneGenero')}</span>
                                            </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: 10 }}>
                                    <div style={{ position: 'relative' }}>
                                        ...
                                        <input className="wizard-input" style={{ paddingLeft: 40 }} type="text" placeholder={`${t('placeholderCep')} *`} value={formData.cep} onChange={e => { const val = maskCep(e.target.value); setFormData(p => ({ ...p, cep: val })); if (val.length === 9) fetchAddress(val); }} />
                                    </div>
                                    <div style={{ position: 'relative' }}>
                                        <input className="wizard-input" type="text" placeholder={`${t('placeholderCidadeEstado')} *`} value={formData.location} readOnly style={{ background: 'rgba(255,255,255,0.02)', color: '#94a3b8' }} />
                                    </div>
                                </div>
                                <div style={{ position: 'relative' }}>
                                    <input className="wizard-input" type="text" placeholder={`${t('placeholderEndereco')} *`} value={formData.address} onChange={e => setFormData(p => ({ ...p, address: e.target.value }))} readOnly={!!formData.address} style={{ background: formData.address ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.05)', color: formData.address ? '#94a3b8' : '#f1f5f9' }} />
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', gap: 10 }}>
                                    <div style={{ position: 'relative' }}>
                                        <input className="wizard-input" type="text" placeholder={`${t('placeholderNumero')} *`} value={formData.addressNumber} onChange={e => setFormData(p => ({ ...p, addressNumber: e.target.value.replace(/\D/g, '') }))} />
                                    </div>
                                    <div style={{ position: 'relative' }}>
                                        <input className="wizard-input" type="text" placeholder={t('placeholderComplemento')} value={formData.complement} onChange={e => setFormData(p => ({ ...p, complement: e.target.value }))} />
                                    </div>
                                </div>
                                <div style={{ position: 'relative' }}>
                        ...
                        {step === 2 && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 4 }}>
                                    ...
                                    >
                                        {submitting ? (
                                            <><div style={{ width: 18, height: 18, border: '3px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />{t('enviando')}</>
