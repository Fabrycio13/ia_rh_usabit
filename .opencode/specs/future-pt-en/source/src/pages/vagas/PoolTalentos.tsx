...
export const PoolTalentos = () => {
    ...
    const handleConfirmAnalyze = async () => {
        ...
        if (!vaga) { toast.error(t('erroCarregarVaga')); setAnalyzing(false); return; }
        ...
        try {
            ...
            toast.success(`${t('candidatoAnalisado')} "${vaga.title}"`);
            closeAnalyzeModal();
            ...
            {paginated.length === 0 ? (
                        ...
                        <div style={{ background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border)', padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                <button onClick={() => goTo(page - 1)} disabled={page === 1}
                                    ...
                                    <ChevronLeft size={15} /> {t('anterior')}
                                </button>
                                <button onClick={() => goTo(page + 1)} disabled={page === totalPages}
                                    ...
                                    {t('proximo')} <ChevronRight size={15} />
                                </button>
                            </div>
                        </div>
                    )}
                ...
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: gridColumns, padding: '12px 24px', background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
                        {[t('rank'), t('candidato'), t('localizacao'), t('dataEntrada'), t('genero'), t('status'), t('acoes')].map((h, i) => (
                                ...
                                <div key={candidato.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                    <div onClick={() => fetchCandidateDetail(candidato)}
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                                            {(candidato.resume_url || candidato.resume_file_name) ? (
                                                <button title={candidato.resume_file_name || t('curriculo')} onClick={(e) => { e.stopPropagation(); handleViewResume(candidato.resume_url!); }}
                                            ...
                                            <button title={t('analisarParaVaga')} onClick={(e) => { e.stopPropagation(); openConfirmModal(candidato); }}
                    ...
                    {totalPages > 1 && (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderTop: '1px solid var(--border)' }}>
                            <span style={{ fontSize: 13, color: '#64748b' }}>{t('pagina')} {page} de {totalPages} · {filteredCandidatos.length} {t('candidatos').toLowerCase()}</span>
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                <button onClick={() => goTo(page - 1)} disabled={page === 1}
                ...
                <>
                    ...
                    <div style={{
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
                            {loadingVagas ? (
                                <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                                    ...
                                    <p style={{ fontSize: 13, margin: 0 }}>{t('carregando')}</p>
                                </div>
                            ) : vagas.length === 0 ? (
                                <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                                    <p style={{ fontSize: 13, margin: 0 }}>{t('nenhumaVagaDisponivel')}</p>
                                </div>
                            ) : (
                                vagas
                        ...
                        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                            ...
                            >
                                {analyzing ? (
                                    <><Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> {t('analisando')}</>
                                ) : (
                                    <>{t('analisar')}</>
                                )}
                            </button>
