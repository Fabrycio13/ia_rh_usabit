...
export function PipelineLinkSection({
    ...
    async function handleLink() {
        ...
        try {
            ...
            if (alreadyLinked) {
                toast('Candidato já está neste pipeline');
                return;
            }
            ...
            setSelectedId('');
            setDropdownOpen(false);
            toast.success(`${candidateName} vinculado ao pipeline`);
        } catch (err: unknown) {
                ...
                <div className="pls-container" ref={dropdownRef}>
                    ...
                    >
                        <span style={{ color: selectedPipeline ? 'var(--text-main)' : 'var(--text-dim)' }}>
                            {selectedPipeline ? selectedPipeline.name : 'Vincular a outro pipeline...'}
                        </span>
