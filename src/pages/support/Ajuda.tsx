...
const FaqItem = ({ q, a }: { q: string; a: string }) => {
    ...
    return (
        <div
            style={{ borderBottom: '1px solid var(--border)', overflow: 'hidden', transition: 'all 0.2s' }}
        >
            <button
                onClick={() => setOpen(o => !o)}
            ...
            >
                <span style={{ color: 'var(--text-main)', fontSize: '14px', fontWeight: 500 }}>{q}</span>
                {open
            ...
            {open && (
                <p style={{ color: 'var(--text-dim)', fontSize: '14px', lineHeight: '1.65', margin: '0 0 16px', paddingRight: '24px' }}>{a}</p>
            )}
        </div>
    );
};
                ...
                <div>
                    ...
                    <div style={{ position: 'relative', marginBottom: '24px' }}>
                        ...
                        <input
                            ...
                            style={{ width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '11px 14px 11px 42px', color: 'var(--text-main)', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }}
                    ...
                    {filtered.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-dim)' }}>
                            <p style={{ fontSize: '14px', margin: 0 }}>Nenhuma dúvida encontrada para "<strong style={{ color: 'var(--text-main)' }}>{search}</strong>"</p>
                        </div>
                    ) : (
                        filtered.map(cat => (
                            <div key={cat.category} className="faq-section" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '4px 24px', marginBottom: '16px' }}>
                                <p style={{ color: 'var(--primary)', fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', margin: '18px 0 4px' }}>{cat.category}</p>
