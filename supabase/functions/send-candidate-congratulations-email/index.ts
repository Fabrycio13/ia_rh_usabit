...
serve(async (req) => {
  ...
  try {
    ...
    if (!candidateEmail || !vagaId) {
      return new Response(
        JSON.stringify({ error: 'candidateEmail e vagaId são obrigatórios' }),
    ...
    // Rate limit por IP
    const allowed = await checkRateLimit(supabaseAdmin, `ip:${clientIp}`, 'send-candidate-congratulations-email', RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS);
    if (!allowed) {
    ...
    // 4. Buscar candidatura no banco (nome do candidato + confirmação de existência)
...
<div style="margin-top: 32px; padding-top: 32px; border-top: 1px solid rgba(255, 255, 255, 0.1); text-align: center;">
    ...
    safeEdgeError('[send-candidate-congratulations-email] Error:', 'Error:', error);
    return new Response(
