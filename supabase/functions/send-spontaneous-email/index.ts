...
// Envia e-mail de confirmaÃ§Ã£o para candidato que se cadastrou no "Trabalhe Conosco"
// Recebe apenas candidateId (busca dados no banco, evita uso arbitrÃ¡rio)
...
serve(async (req) => {
  ...
  try {
    ...
    if (!candidateId) {
      return new Response(
        JSON.stringify({ error: 'candidateId Ã© obrigatÃ³rio' }),
    ...
    if (candidateError || !candidate) {
      return new Response(
        JSON.stringify({ error: 'Candidato nÃ£o encontrado' }),
    ...
    if (!RESEND_API_KEY) {
      console.log('[send-spontaneous-email] Email nÃ£o enviado (RESEND_API_KEY nÃ£o configurada)');
      return new Response(
...
<head>
    ...
    <title>CurrÃ­culo Recebido</title>
        ...
        <div style="max-width: 600px; width: 100%; margin: 0 auto; background: #0b111a; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 24px; padding: 40px 24px; text-align: left; box-sizing: border-box;">
            ...
            <div style="background: linear-gradient(135deg, rgba(44, 88, 253, 0.15) 0%, transparent 100%); border-radius: 20px; padding: 2px; margin-bottom: 32px;">
                <div style="background: #0b111a; border-radius: 18px; padding: 32px;">
                    ...
                    <p style="font-size: 17px; line-height: 1.6; color: #ffffff; margin: 0; font-weight: 500;">
                        Recebemos seu currÃ­culo com sucesso!
                    </p>
                </div>
            </div>
            ...
            <div style="color: #94a3b8; font-size: 16px; line-height: 1.7; margin-bottom: 40px;">
                <p style="margin: 0 0 20px;">
                    Agradecemos pelo envio do seu currÃ­culo.
                </p>

                <p style="margin: 0 0 20px;">
                    Seu perfil foi incluÃ­do em nosso <strong style="color:#ffffff;">Banco de Talentos</strong> e poderÃ¡ ser considerado para futuras oportunidades compatÃ­veis com sua experiÃªncia e qualificaÃ§Ãµes.
                </p>

                <p style="margin: 0 0 20px;">
                    Sempre que surgirem vagas aderentes ao seu perfil, nossa equipe poderÃ¡ entrar em contato.
                </p>
