...
// Envia e-mail de agradecimento para candidatos não selecionados quando a vaga é fechada
...
serve(async (req) => {
  ...
  try {
    ...
    // Rate limit por IP
    const allowed = await checkRateLimit(supabaseAdmin, `ip:${clientIp}`, 'send-candidate-thankyou-email', RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS);
    if (!allowed) {
    ...
    if (!RESEND_API_KEY) {
      console.log('[send-candidate-thankyou-email] Email não enviado (RESEND_API_KEY não configurada)');
    ...
    const html = buildEmailHtml({
      title: 'Retorno do Processo Seletivo',
...
<div style="margin-top: 32px; padding-top: 32px; border-top: 1px solid rgba(255, 255, 255, 0.1); text-align: center;">
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Thankyou email sent' }),
      { status: 200, headers: { ...getCorsHeaders(origin), 'Content-Type': 'application/json' } }
    );

  } catch (error: unknown) {
    safeEdgeError('[send-candidate-thankyou-email] Error:', 'Error:', error);
    return new Response(
