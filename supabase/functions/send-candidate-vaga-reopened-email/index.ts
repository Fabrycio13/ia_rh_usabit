// Edge Function: send-candidate-vaga-reopened-email
// Envia e-mail de notificação para candidatos quando a vaga cancelada é reaberta
...
serve(async (req) => {
  ...
  try {
    const { candidateEmail, vagaId }: { candidateEmail: string; vagaId: string } = await req.json();

    if (!candidateEmail || !vagaId) {
      return new Response(JSON.stringify({ error: 'candidateEmail e vagaId são obrigatórios' }), {
        status: 400, headers: { ...getCorsHeaders(origin), 'Content-Type': 'application/json' },
      });
    }

    if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
      return new Response(JSON.stringify({ error: 'Configuração de banco ausente' }), {
    ...
    // Rate limit por IP
    const allowed = await checkRateLimit(supabaseAdmin, `ip:${clientIp}`, 'send-candidate-vaga-reopened-email', RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS);
    if (!allowed) {
    ...
    if (profileError || !profile || !ALLOWED_ROLES.includes(profile.user_role)) {
    ...
    // 3. Validar que a vaga pertence à organização do caller
    ...
    if (profile.user_role !== 'owner' && vaga.organization_id !== profile.organization_id) {
      return new Response(JSON.stringify({ error: 'Vaga não pertence à sua organização' }), {
        status: 403, headers: { ...getCorsHeaders(origin), 'Content-Type': 'application/json' },
      });
    }

    // 4. Buscar candidatura no banco
    ...
    if (!RESEND_API_KEY) {
      console.log('[send-candidate-vaga-reopened-email] Email não enviado (RESEND_API_KEY não configurada)');
...
<div style="margin-top: 32px; padding-top: 32px; border-top: 1px solid rgba(255, 255, 255, 0.1); text-align: center;">
    ...
    if (!res.ok) {
      ...
      safeEdgeError('Resend error:', error);
      return new Response(
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Reopened email sent' }),
      { status: 200, headers: { ...getCorsHeaders(origin), 'Content-Type': 'application/json' } }
    );

  } catch (error: unknown) {
    safeEdgeError('[send-candidate-vaga-reopened-email] Error:', 'Error:', error);
    return new Response(
